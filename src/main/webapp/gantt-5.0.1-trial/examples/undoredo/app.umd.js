...
class ActionsGrid extends TreeGrid {
  ...
  onRecordingStop({
    ...
    var actionStore = this.store,
      ...
      children: transaction.queue.map((action, idx) => {
        ...
        if (type === 'UpdateAction' && model instanceof ProjectModel) {
          ...
          var fromEvent = model.fromEvent || this.taskStore.getById(model.from),
    ...
    this.selectedRecord = action; // Remove after because the selection insists on having at least one selected record
  ...
  onRestoringStop({
    ...
    var action = this.store.rootNode.children[stm.position];
...
var project = window.project = new ProjectModel({
  ...
  stm,
  taskStore
} = project; // Primary application container. Contains two widgets: Gantt and Actions grid.
