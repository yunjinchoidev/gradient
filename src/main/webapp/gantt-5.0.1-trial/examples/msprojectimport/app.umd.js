...
class Importer {
  ...
  async importData(data) {
    ...
    project = new me.gantt.projectModelClass({
      silenceInitialCommit: false // To save imported data provide `sync` url and set `autoSync` true, or call `gantt.project.sync()` manually after data is imported.
      // autoSync             : true,
      // transport            : {
      //     sync : {
      //         url : 'syncUrl'
    ...
    me.importProject(data); // Assign the new project to the gantt before launching commitAsync()
  ...
  processResource(data) {
    ...
    var resource = new this.resourceStore.modelClass(data);
  ...
  processDependency(data) {
    ...
    delete data.id;
    var dep = new me.dependencyStore.modelClass(data);
  ...
  processTask(data) {
    ...
    var t = new me.taskStore.modelClass(data);
  ...
  processCalendar(data) {
    ...
    delete data.children;
    delete data.id;
    var t = new me.calendarManager.modelClass(Object.assign(data, {
      intervals
    }));
  ...
  importColumns(data) {
    var columns = data.columns.map(this.processColumn, this).filter(column => column);
    var columnStore = this.gantt.subGrids.locked.columns; // if no columns extracted apply default set (if configured)
  ...
  processColumn(data) {
    var columnClass = this.gantt.columns.constructor.getColumnClass(data.type); // ignore unknown columns (or columns that classes are not loaded)
  ...
  tbar: [{
    ...
    onClick: () => {
      ...
      if (files) {
        ...
        formData.append('mpp-file', files[0]); // test environment support
        ...
        AjaxHelper.post('php/load.php', formData, {
          ...
          if (parsedJson.success && parsedJson.data) {
            var {
              project
            } = gantt; // Import the uploaded mpp-file data (will make a new project and assign it to the gantt)

            await importer.importData(parsedJson.data); // destroy old project

            project.destroy(); // set the view start date to the loaded project start

            gantt.setStartDate(gantt.project.startDate);
            await gantt.scrollToDate(gantt.project.startDate, {
              block: 'start'
            });
            input.clear(); // remove "Importing project ..." mask
