...
try {
    // for testing - simply use the mpp-file shipped w/ the demo
    if ($_REQUEST['test']) {
        $move_path = "${tmp_dir}AdvancedExample.mpp";

        if (!copy(dirname(__DIR__) .'/AdvancedExample.mpp', $move_path)) {
            throw new Exception('Cannot save file. Please verify, that web-server user account has write permission to '. $move_path);
        }

    // normal case - use the uploaded file
    } else {
        if (!$_FILES['mpp-file']) {
