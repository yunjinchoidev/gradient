...
class BackendStorage {
  constructor() {
    ...
    _defineProperty(this, "key", 'bryntum-state:backend-storage');
...
class AsyncStorage extends StateStorage {
  constructor(...args) {
    ...
    _defineProperty(this, "storage", new BackendStorage());
  ...
  setItem(key, value) {
    this.data[key] = value; // Notify underlying storage to persist change to remote resource
  ...
  removeItem(key) {
    delete this.data[key]; // Notify underlying storage to persist change to remote resource
...
async function initAsyncStorage() {
  var asyncStorage = new AsyncStorage();
...
initAsyncStorage().then(storage => {
  ...
  var gantt = new Gantt({
    ...
    columns: [// To persist column state it is better to explicitly specify an id
    {
      id: 'name',
      type: 'name',
      width: 250
