...
StartTest(t => {
    ...
    function assertLines(t) {
        ...
        t.subTest('Asserting all the lines', t => {
            const
                tasks = gantt.taskStore.getRange(),
    ...
    t.it('Progress line should react on task store events', async t => {
        ...
        const linesCount = document.querySelectorAll('.b-gantt-progress-line').length;
    ...
    t.it('Should redraw progress line after cancelled drag drop', async t => {
        ...
        t.chain(
            ...
            async() => {
                await Promise.all([
                    gantt.await('progressLineDrawn', false),
                    t.type(null, '[ESC]')
                ]);
                assertLines(t);
            }
    ...
    t.it('Should paint line correctly for milestones', async t => {
        await setup({
            project : {
                ...
                tasksData : [
                    { id : 11, name : 'Task 11', startDate : '2019-01-14', duration : 0 },
                    { id : 12, name : 'Task 12', startDate : '2019-02-05', duration : 0, percentDone : 100 },
                    { id : 13, name : 'Task 13', startDate : '2019-02-24', duration : 0, percentDone : 99.9 },
                    { id : 14, name : 'Task 14', startDate : '2019-02-24', duration : 0, percentDone : 100 }
                ]
            }
        });

        await t.waitForSelector('line[data-task-id=11]');
        // Safari needs time to draw all lines
        await t.waitForAnimationFrame();

        // Milestones are either 0 progress or fully done so we draw to either left edge or right edge of the diamond
        const
            task11Lines = t.query('line[data-task-id=11]'),
        ...
        t.isApprox(
            parseInt(task11Lines[1].getAttribute('x1'), 10),
            gantt.timeAxisViewModel.getPositionFromDate(gantt.taskStore.getById(11).startDate) - 15,
            'Line should draw to milestone start '
        );
        ...
        t.isApprox(
            parseInt(task12Lines[0].getAttribute('x1'), 10),
            gantt.timeAxisViewModel.getPositionFromDate(statusDate) - 15,
            'Line should draw to statusDate for completed task'
        );
        ...
        t.isApprox(
            parseInt(task13Lines[1].getAttribute('x1'), 10),
            gantt.timeAxisViewModel.getPositionFromDate(gantt.taskStore.getById(13).startDate) - 15,
            'Line should draw to milestone start '
        );

        t.selectorCountIs('line[data-task-id=14]', 1, 'vertical straight line for task 14 since it is completed');
        t.isApprox(
            parseInt(task14Lines[0].getAttribute('x1'), 10),
            gantt.timeAxisViewModel.getPositionFromDate(statusDate) - 15,
            'Line should draw to statusDate for completed task'
        );
    });
    ...
    t.it('Should not paint progress for inactive tasks', async t => {
        await setup({
            project : {
                ...
                tasksData : [
                    { id : 11, name : 'Task 11', startDate : '2019-01-14', duration : 2 },
                    { id : 12, name : 'Task 12', startDate : '2019-01-14', duration : 2, percentDone : 100 },
                    { id : 13, name : 'Task 13', startDate : '2019-01-14', duration : 2, inactive : true },
                    { id : 14, name : 'Task 14', startDate : '2019-01-14', duration : 2, inactive : true }
                ]
            }
        });
        ...
        t.isApprox(
            parseInt(task11Lines[1].getAttribute('x1'), 10),
            gantt.timeAxisViewModel.getPositionFromDate(gantt.taskStore.getById(11).startDate),
            'Line should draw to the task start since it`s not started yet'
        );
        t.is(task11Lines[0].getAttribute('x2'), task11Lines[1].getAttribute('x1'), '2 task lines should meet at same point');
        ...
        t.isApprox(
            parseInt(task12Lines[0].getAttribute('x1'), 10),
            gantt.timeAxisViewModel.getPositionFromDate(statusDate),
            'Line should draw to statusDate for completed task'
        );

        t.selectorCountIs('line[data-task-id=13]', 1, 'vertical straight line for task 13 since it is inactive');
        t.isApprox(
            parseInt(task13Lines[0].getAttribute('x1'), 10),
            gantt.timeAxisViewModel.getPositionFromDate(statusDate),
            'Line should draw to statusDate for inactive task'
        );
