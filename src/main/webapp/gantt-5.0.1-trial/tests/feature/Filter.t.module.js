...
StartTest(t => {
            ...
            tasksData : [
                {
                    ...
                    children  : [
                        ...
                        { id : 14, name : 'Task 14', startDate : '2020-02-24', duration : 3 }
                    ]
                }
            ],
    ...
    t.it('Should not display deleted data after re-apply filter', async t => {
        ...
        t.chain(
            ...
            { waitForSelectorNotFound : `${sel}[data-task-id="11"]`, desc : 'Task 11 has been deleted' },
            (next) => {
                gantt.on('renderRows', () => {
                    next();
                });
            ...
            { waitForSelectorNotFound : `${sel}[data-task-id="11"]`, desc : 'Task 11 has not been showed again after filter re-apply' }
        );
    });
    ...
    t.it('Should not close filter popup when field trigger is clicked', async t => {
        ...
        t.chain(
            ...
            { type : '2[ENTER]', clearExisting : true },
            next => {
            ...
            { waitForSelectorNotFound : '.b-filter-popup', desc : 'Popup closed after click out of the filter popup' }
        );
    });
    ...
    t.it('Should apply filter to date column', async t => {
        ...
        await Promise.all([
            gantt.taskStore.await('filter'),
            t.click('.b-menuitem:contains("On")')
        ]);
    ...
    t.it('Should apply filter to WBS column from context menu', async t => {
        ...
        await Promise.all([
            gantt.taskStore.await('filter'),
            t.click('.b-menuitem:contains("Equals")')
        ]);
    ...
    t.it('Should apply filter to WBS column from text field', async t => {
        ...
        await Promise.all([
            gantt.taskStore.await('filter'),
            t.type('.b-textfield input', '1.1[ENTER]')
        ]);
