...
StartTest(t => {
  ...
  t.it('Should disable certain fields for parent tasks', t => {
    ...
    t.chain({
      ...
      const {
        ...
        endDate,
        percentDone
      } = gantt.features.taskEdit.editor.widgetMap;
  ...
  t.it('Should preserve scroll when cancelling changes', async t => {
    ...
    const task = gantt.taskStore.getAt(gantt.taskStore.count - 1);
    let scroll;
    t.chain({
  ...
  t.it('autoSync', async t => {
    ...
    }, // Syncing is on a timer, so wait for it to cycle
    {
      ...
      t.is(syncCallCount, 1, 'sync count is the same'); // Cancel editing
  ...
  t.it('Task editor should be placed correctly for unscheduled task', async t => {
    ...
    t.chain({
      ...
      const editRect = t.rect('.b-gantt-taskeditor'),
  ...
  t.it('Task editor should allow to type in Duration for unscheduled task', async t => {
    ...
    await gantt.scrollTaskIntoView(added);
    await gantt.editTask(added);
    const widgetMap = gantt.features.taskEdit.editor.widgetMap;
    t.chain({
      waitForSelector: '.b-gantt-taskeditor'
    }, {
  ...
  t.it('Should cancel editing in an orderly manner if editor hidden', async t => {
    ...
    await t.doubleClick('.b-gantt-task.id11'); // It must show after being hidden
  ...
  t.it('Should not crash if project data is replaced when editor is open', async t => {
    ...
    t.ok(gantt.features.taskEdit.editor.hidden, 'Editor hid itself as a reaction to record no longer being part ' + 'of the taskStore');
  ...
  t.it('Should be able to hide items on the form', async t => {
    ...
    t.chain({
      ...
      waitForSelectorNotFound: '[name=calendar]',
      desc: 'calendarField is not visible'
    }, {
      waitForSelectorNotFound: '[name=manuallyScheduled]',
      desc: 'manuallyScheduledField is not visible'
    }, {
      waitForSelectorNotFound: '[name=schedulingMode]',
      desc: 'schedulingModeField is not visible'
    }, {
      waitForSelectorNotFound: '[name=effortDriven]',
      desc: 'effortDrivenField is not visible'
    }, {
      waitForSelectorNotFound: '.b-divider[data-text=Constraint]',
      desc: 'divider is not visible'
    }, {
      waitForSelectorNotFound: '[name=constraintType]',
      desc: 'constraintTypeField is not visible'
    }, {
      waitForSelectorNotFound: '[name=constraintDate]',
      desc: 'constraintDateField is not visible'
    }, {
      waitForSelectorNotFound: '[name=rollup]',
      desc: 'rollupField is not visible'
    });
  ...
  t.it('Should not throw an exception when clicking DELETE button', async t => {
    ...
    t.chain({
      ...
      desc: 'Shifted start date'
    }, {
      click: 'button:textEquals(Delete)',
      desc: 'Clicked DELETE button'
    }, {
      click: '.b-messagedialog button:textEquals(Delete)',
      desc: 'Clicked YES button'
    }, {
      waitForSelectorNotFound: '.b-taskeditor',
      desc: 'task editor closed'
    }, () => {
  ...
  t.it('Should filter all items correctly on grid editor when set filterOperator on the fly', async t => {
    ...
    t.chain({
      ...
      waitForSelector: '.b-list-item:textEquals(Customer approval (3))',
      desc: 'First item found.'
    }, {
      waitForSelector: '.b-list-item:textEquals(Follow up with customer (234))',
      desc: 'Second item found.'
  ...
  t.it('Should reset successors and predecessor if click on cancel button on editor', async t => {
    ...
    t.chain({
      ...
      waitForSelector: `.b-predecessors-tab .b-number-cell[data-column="fullLag"]:contains(${predecessor.lag} days)`,
      desc: 'Predecessor lag cell was not changed'
      ...
      waitForSelector: `.b-successors-tab .b-number-cell[data-column="fullLag"]:contains(${successor.lag} days)`,
      desc: 'Successor lag cell was not changed'
  ...
  t.it('Should hide task editor when task bar scrolls out of view', async t => {
    ...
    await t.waitFor(() => {
      var _gantt$features$taskE;

      return (_gantt$features$taskE = gantt.features.taskEdit.editor) === null || _gantt$features$taskE === void 0 ? void 0 : _gantt$features$taskE.containsFocus;
    }); // Scroll event bar out of view

    await gantt.subGrids.normal.scrollable.scrollBy(300); // Editor must hide.
