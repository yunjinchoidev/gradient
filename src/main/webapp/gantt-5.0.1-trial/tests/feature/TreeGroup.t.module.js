import { DateHelper, Rectangle } from '../../build/gantt.module.js?457330';
...
StartTest(t => {
    ...
    t.it('Should group initially', async t => {
        ...
        t.is(strings[10], 'Step 2', 'Row 10 correct');

        // Correct task bar count etc
        ...
        t.selectorCountIs('.b-sch-dependency', 10, 'Dependencies drawn');

        // Assert position for one bar from each parent
        t.isApproxRect(rect('[data-task-id="11"]'), { top : 56, left : 23, width : 234, height : 25 }, 'Investigate');
        t.isApproxRect(rect('[data-task-id="21"]'), { top : 286, left : 257, width : 117, height : 25 }, 'Preparation work');
        t.isApproxRect(rect('[data-task-id="234"]'), { top : 562, left : 445, width : 23, height : 25 }, 'Follow up with customer');
    ...
    t.it('Should regroup at runtime', async t => {
        ...
        t.selectorCountIs('.b-gantt-task-parent', 9, '9 parents');

        // Assert position for one bar from each parent
        t.isApproxRect(rect('[data-task-id="11"]'), { top : 194, left : 23, width : 234, height : 25 }, 'Investigate');
        t.isApproxRect(rect('[data-task-id="21"]'), { top : 792, left : 257, width : 117, height : 25 }, 'Preparation work');
        t.isApproxRect(rect('[data-task-id="234"]'), { top : 470, left : 445, width : 23, height : 25 }, 'Follow up with customer');
    });
    ...
    t.it('Should restore original data when clearing groups', async t => {
        ...
        t.is(strings[10], 'Step 1', 'Row 10 correct');

        // Assert position for one bar from each parent
        t.isApproxRect(rect('[data-task-id="11"]'), { top : 102, left : 23, width : 234, height : 25 }, 'Investigate');
        t.isApproxRect(rect('[data-task-id="21"]'), { top : 332, left : 257, width : 117, height : 25 }, 'Preparation work');
        t.isApproxRect(rect('[data-task-id="234"]'), { top : 608, left : 445, width : 23, height : 25 }, 'Follow up with customer');
    });
