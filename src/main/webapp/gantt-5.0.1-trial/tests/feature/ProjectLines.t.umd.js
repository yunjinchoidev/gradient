...
StartTest(t => {
  ...
  t.it('Should update project lines correctly', t => {
    ...
    function checkDates(startDate, endDate) {
      const [start, end] = gantt.features.projectLines.store.getRange();
    ...
    }, async () => checkDates(new Date(2017, 0, 16), new Date(2017, 0, 26)), async () => {
  ...
  t.it('Should update project lines correctly after undo', async t => {
    ...
    const project = gantt.project,
          stm = project.getStm();
    let originalProjectStartDate, originalProjectEndDate;
  ...
  t.it('Should show project lines correctly after initial load', async t => {
    ...
    t.chain({
      ...
      const startLine = t.$('.b-gantt-project-line:contains(start)')[0],
  ...
  t.it('Feature should react on project change', async t => {
    ...
    await t.waitFor({
      method() {
        const task = t.rect('.b-gantt-task-wrap'),
              [startLine, endLine] = Array.from(document.querySelectorAll('.b-sch-foreground-canvas .b-sch-timerange')).map(el => t.rect(el));
