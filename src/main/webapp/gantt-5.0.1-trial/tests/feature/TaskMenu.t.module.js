import { DateHelper, ObjectHelper } from '../../build/gantt.module.js?457330';
...
StartTest(t => {
    ...
    t.it('Basic context menu works', async t => {
        ...
        const listeners = {
            taskmenubeforeshow : ({ source, items, taskRecord }) => {
            ...
            taskmenushow : ({ source, taskRecord, taskElement }) => {
            ...
            taskmenuitem : ({ source, item, taskRecord }) => {
        ...
        t.chain(
            ...
            { moveMouseTo : '.b-menuitem:contains(Add)', offset : ['99%', 1] },
            ...
            { moveMouseTo : '.b-menuitem:contains(Add)', offset : ['99%', 1] },
            { click : '.b-icon-down', desc : 'Add task below' },
            next => {
                const
                    { added } = gantt.taskStore.changes,
    ...
    t.it('Should disable "Indent" menu item for the first child', async t => {
        ...
        t.chain(
            ...
            { waitForSelector : '.b-menuitem.b-disabled[data-ref="indent"]', desc : 'Menu item is disabled' },
            { type : '[ESC]' },
            { waitForSelectorNotFound : '.b-menu', desc : 'Menu is hidden' },
            { rightClick : '[data-task-id="2"]' },
            { waitForSelector : '.b-menuitem[data-ref="indent"]', desc : 'Menu item is present' },
            () => {
    ...
    t.it('Should disable "Outdent" menu item for the first level tasks', async t => {
        ...
        t.chain(
            { rightClick : '[data-task-id="1000"]' },
            { waitForSelector : '.b-menuitem.b-disabled[data-ref="outdent"]', desc : 'Menu item is disabled' },
            { type : '[ESC]' },
            { waitForSelectorNotFound : '.b-menu', desc : 'Menu is hidden' },
            { rightClick : '[data-task-id="2"]' },
            { waitForSelector : '.b-menuitem[data-ref="outdent"]', desc : 'Menu item is present' },
            () => {
    ...
    t.it('Should show task menu for empty part of timeaxis', async t => {
        ...
        t.chain(
            { rightClick : '.b-sch-timeaxis-cell', offset : [5, '50%'] },
            { waitForSelector : '.b-menu', desc : 'Menu item is disabled' },
            { type : '[ESC]' },
            { waitForSelectorNotFound : '.b-menu', desc : 'Menu is hidden' }
        );
    });
    ...
    t.it('Sanity check of default subitems', async t => {
        ...
        t.chain(
            { contextmenu : '.b-gantt-task.id11', desc : 'First menu is cancelled' },
            { waitForSelector : '.b-menu-text', desc : 'A menu item is on the screen' },
            async() => {
            ...
            { waitForSelector : '.b-sub-menu .b-menu-text', desc : 'A submenu item is on the screen' },
            async() => {
    ...
    t.it('Sanity check of processItems', async t => {
        gantt = await t.getGanttAsync({
            features : {
                taskMenu : {
                    processItems : ({ items, taskRecord, column, event }) => {
    ...
    t.it('Should support being shown programmatically', async t => {
        gantt = await t.getGanttAsync({
            ...
            listeners : {
                cellclick({ event }) {
                    gantt.showContextMenu(event, { anchor : true, target : event.target, align : 'l-r' });
                }
            }
        });
    ...
    t.it('Should use icons defined with b-icon', async t => {
        ...
        t.chain(
            ...
            { waitForElementVisible : '.b-icon-indent', desc : 'b-icon applied for indent item menu' },
            { waitForElementVisible : '.b-icon-outdent', desc : 'b-icon applied for outdent item menu' },
            next => {
            ...
            { waitForElementVisible : '.b-icon-milestone:visible', desc : 'b-icon applied for milestone item menu' },
