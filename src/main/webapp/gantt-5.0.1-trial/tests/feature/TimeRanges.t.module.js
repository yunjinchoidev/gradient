...
StartTest(t => {
    ...
    t.it('Should create store with correct modelClass', t => {
        const gantt = new Gantt({
            project : {}
        });

        t.ok(gantt.project.timeRangeStore.modelClass.isTimeSpan, 'Correctly using TimeSpan');
    });
    ...
    t.it('Should show time ranges from the project timeRangeStore', t => {
        ...
        t.chain(
            ...
            (next) => {
                t.selectorCountIs('.b-grid-headers .b-sch-nonworkingtime', 5, '5 weekends in header');
                t.selectorCountIs('.b-grid-headers .b-sch-timerange.b-sch-range', 5, '5 weekend range elements in header');
    ...
    // https://github.com/bryntum/support/issues/3204
    t.it('Should show inline timeRangesData', async t => {
        gantt = t.getGantt({
            appendTo  : document.body,
            startDate : new Date(2017, 0, 15),
            endDate   : new Date(2017, 1, 10),
        ...
        await t.waitForSelector('.b-grid-headers .b-sch-line label:contains(Cool line)');
    });
