...
StartTest(t => {
  ...
  t.it('Creating dependencies with D&D', async t => {
    ...
    {
      waitFor: () => t.rect('.b-sch-dependency-creation-tooltip .b-popup-header').width >= 16,
      desc: 'Header tooltip title content is visible when dragging from source dependency'
    }, {
      moveCursorTo: '[data-task-id=\'13\'] .b-gantt-task .b-sch-terminal-left'
    }, // https://github.com/bryntum/support/issues/1388
    {
      waitFor: () => t.rect('.b-sch-dependency-creation-tooltip .b-popup-header').width >= 16,
      desc: 'Header tooltip title content is visible when dragging to target dependency'
    }, {
      mouseUp: null
    }, {
  ...
  t.it('Editing dependencies', async t => {
    ...
    predecessors.sort((a, b) => a.id - b.id); // Initial values correct
    ...
    t.chain({
      ...
      trigger: {
        doubleclick: () => gantt.getCell(multiDepTaskPredecessorCellCtx)
      }
    }, next => {
      ...
      trigger: {
        click: () => predecessorField.triggers.expand.element
      }
    }, next => {
    ...
    next => {
      ...
      trigger: {
        click: () => predecessorPicker.triggers.expand.element
      }
    }, // Select the SS item
    {
    ...
    }, // This will finish dependency type editing and start propagation, plus it will start full lag editing
    {
      ...
      waitForPropagate: gantt
    }, async () => durationField = dependencyGrid.features.cellEdit.editorContext.editor.inputField, // Make it +1 day
    {
    ...
    async () => t.ok(dependencyGrid.features.cellEdit.editorContext && dependencyGrid.features.cellEdit.editorContext.editor.containsFocus, 'Changing the lag did not terminate the edit'), // Save the edit
    {
  ...
  t.it('Should redraw on TaskStore sort', async t => {
    ...
    await t.waitFor(() => t.getSVGBox(document.querySelector('[depId="1"]')).top > 200, () => t.pass('Dependencies redrawn'));
  });
  ...
  t.it('Should update dependency type correctly', async t => {
    ...
    }, // make sure we mouseenter the task on the next step
    {
      ...
      dep.type = 0;
      await Promise.all([gantt.project.commitAsync(), gantt.await('dependenciesDrawn')]);
