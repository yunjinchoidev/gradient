...
StartTest(t => {
  ...
  t.it('Should be able to tab through all cells while editing', t => {
    gantt = t.getGantt({
      columns: Object.values(AllColumns).map(ColumnClass => ({
        type: ColumnClass.type
      }))
    });
    const editableColumns = gantt.columns.visibleColumns.filter(c => gantt.features.cellEdit.getEditingContext({
      row: 0,
      column: c
    }));
    t.chain({
      ...
      type: '[TAB]'.repeat(editableColumns.length * 3)
    }, // All editable cells in three rows
    () => {
  ...
  t.it('Should be able to tab through all cells for new record', t => {
    ...
    t.chain({
      ...
      type: '[TAB]'.repeat(editableColumns.lengtht * 2)
    }, // All editable cells in two rows
    () => {
      t.pass('Tabbed through without exception');
    });
  });
  t.it('Should validate dependencies', async t => {
    ...
    const task1 = gantt.taskStore.getById(1),
    ...
    {
      type: '2[ENTER]',
      target: () => dependencyField.input
    }, {
      waitForSelector: '.b-dependencyfield.b-invalid',
      desc: 'Editing successors started for task 1'
    ...
    }, // TAB off to trigger update
    {
      ...
      waitFor: () => predecessorsCell.textContent === '3',
      desc: 'Cell content is ok'
  ...
  t.it('Should close dependency editor on escape', async t => {
    ...
    t.chain({
      ...
      waitForSelector: '.b-dependencyfield.b-contains-focus',
      desc: 'Editing predecessors started for task 1'
    }, {
      type: '[ESC]'
    }, {
      waitForSelectorNotFound: '.b-dependencyfield',
      desc: 'Editing predecessors cancelled for task 1'
    }, {
      ...
      waitForSelectorNotFound: '.b-dependencyfield',
      desc: 'Editing successors cancelled for task 1'
    }, {
      dblclick: '.task2 [data-column=predecessors]'
    }, {
      waitForSelector: '.b-dependencyfield.b-contains-focus',
      desc: 'Editing predecessors started for task 2'
    }, {
      type: '[ESC]'
    }, {
      waitForSelectorNotFound: '.b-dependencyfield',
      desc: 'Editing predecessors cancelled for task 2'
    }, {
      dblclick: '.task2 [data-column=successors]'
    }, {
      waitForSelector: '.b-dependencyfield.b-contains-focus',
      desc: 'Editing successors started for task 2'
    }, {
      type: '[ESC]'
    }, {
      waitForSelectorNotFound: '.b-dependencyfield',
      desc: 'Editing successors cancelled for task 2'
    });
  ...
  t.it('should be possible to edit values for an auto added field', t => {
    ...
    next => {
      ...
      waitFor: () => cellEdit.editor.inputField.value === 'qwe'
    ...
    }, // cell value correct
    {
      ...
      waitFor: () => cellEdit.editor.inputField.value === 'asd'
    } // editor value correct
    );
  }); // https://github.com/bryntum/support/issues/53
  ...
  t.it('Should clear invalid state of previous edit', async t => {
    ...
    await t.click('.b-icon-add'); // cell renderer should never output undefined before you have chosen a task
  ...
  t.it('Should not allow setting end before start', async t => {
    gantt = t.getGantt({
      columns: [{
        ...
        id: 'end',
        text: 'normal end date',
        type: 'enddate'
      }, {
        id: 'endWithMin',
        text: 'min date 2022 Jan 1st',
        type: 'enddate',
        editor: {
  ...
  t.it('Should not crash when triggering edit for a record in a collapsed parent, or in a column with Number type id', async t => {
    gantt = await t.getGanttAsync({
      columns: [// NOTE - we should accept column id being integer
      {
        id: 1,
        type: 'name'
      }],
