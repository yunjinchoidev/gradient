StartTest(t => {
    ...
    t.it('Should allow user cut and paste using context menu', async t => {
        ...
        t.chain(
            ...
            { waitForSelector : '.b-cut-row[data-id=11]', desc : 'Row id 11 style was changed to "cut" when using context menu' },
            { waitForSelector : '[data-task-id=11] .b-cut-row', desc : 'Task bar id 11 style was changed to "cut" when using context menu' },
    ...
    t.it('Should allow user multiple cut and paste using CTRL+X and CTRL+V', async t => {
        ...
        t.chain(
            { click : '[data-id=12]' },
            { click : '[data-id=13]', options : { ctrlKey : true } },
            { type : '[X]', options : { ctrlKey : true } },
            {
    ...
    t.it('Should allow user multiple copy and paste using CTRL+C and CTRL+V', async t => {
        ...
        t.chain(
            { click : '[data-id=232]' },
            { click : '[data-id=233]', options : { ctrlKey : true } },
            { type : '[C]', options : { ctrlKey : true } },
            { click : '[data-id=3]' },
            { type : '[V]', options : { ctrlKey : true } },
            () => {
                const
                    [newRecCopy1] = store.findByField('name', 'Step 2 - 2'),
    ...
    t.it('Should keep dependencies via copied tasks using CTRL+C and CTRL+V', async t => {
        ...
        t.chain(
            { click : '[data-id=233]' },
            { click : '[data-id=234]', options : { ctrlKey : true } },
            { type : '[C]', options : { ctrlKey : true } },
            { click : '[data-id=3]' },
            { type : '[V]', options : { ctrlKey : true } },
            () => {
                const
                    [newRecCopy1] = store.findByField('name', 'Step 3 - 2'),
                ...
                t.is(newRecCopy1.data.dependencies[0]?.toEvent.id, newRecCopy2.id, 'Row copy of the id 233 has a dependency to second copied record');
                t.is(newRecCopy2.data.dependencies[0]?.fromEvent.id, newRecCopy1.id, 'Row copy of the id 234 has a dependency to first copied record');
            }
        );
    });
    ...
    t.it('Should keep hierarchy via copied tasks using CTRL+C and CTRL+V', async t => {
        ...
        t.chain(
            { click : '[data-id=23]' },
            { click : '[data-id=231]', options : { ctrlKey : true } },
            { type : '[C]', options : { ctrlKey : true } },
            { click : '[data-id=3]' },
            { type : '[V]', options : { ctrlKey : true } },
            () => {
                const
                    [newRecCopy1] = store.findByField('name', 'Build prototype - 2');

                t.is(newRecCopy1.data.children[0]?.data.name, 'Step 1 - 2', 'Pa copy of the id 23 has a child record with the name of the second record');
            }
        );
    });
    ...
    t.it('Should trigger beforePaste event', async t => {
        ...
        gantt = await t.getGanttAsync({
            listeners : {
                beforePaste : ({ referenceRecord, records, isCut }) => {
        ...
        await t.click('.b-grid-cell:contains(Investigate)');
        await t.type(null, 'x', null, null, { ctrlKey : true });
