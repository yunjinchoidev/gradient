...
StartTest(t => {
  ...
  function assertPercentBarWidth(t, taskRecord) {
    const taskElement = gantt.getElementFromTaskRecord(taskRecord),
  ...
  t.it('Should render percent bars', async t => {
    ...
    t.selectorCountIs('.b-task-percent-bar', taskElements.length, 'One per normal task rendered'); // Check all widths
  ...
  t.it('Should set percent to 0 if dragging fully to the start of the bar', async t => {
    ...
    t.chain({
      ...
      drag: '.foo .b-task-percent-bar-handle',
      by: [-100, 0]
    }, {
  ...
  t.it('Should be possible to resize percent bar to 100% of the task width', async t => {
    ...
    t.chain({
      ...
      drag: '.id11 .b-task-percent-bar-handle',
      by: [400, 0],
      dragOnly: true
    }, async () => {
      const barEl = document.querySelector('.id11 .b-task-percent-bar'),
