import { DateHelper, ProjectGenerator, Timeline, Rectangle } from '../../build/gantt.module.js?457330';
...
StartTest(t => {
    ...
    t.it('Should reschedule project after task reordering', async t => {
        ...
        t.chain(
            { drag : '.id13', to : '.id21', toOffset : [50, 10] },
            {
                waitFor : () => {
                    const
                        task1 = gantt.taskStore.getById(13),
    ...
    t.it('Dragging should drag the real task element', async t => {
        ...
        t.chain(
            { drag : taskSelector, by : [deltaX, 0], dragOnly : true },
    ...
    t.it('Dragging task and moving mouse over the timeline widget should not crash', async t => {
        ...
        await t.dragTo({ source : '.b-gantt-task', target : '.b-timeline' });
    ...
    t.it('Should be possible to use custom tooltipTemplate', async t => {
        gantt = await t.getGanttAsync({
            features : {
                taskDrag : {
                    ...
                    tooltipTemplate : ({ taskRecord, startDate : newStartDate, endDate : newEndDate }) => {
        ...
        const { startDate, endDate } = gantt.taskStore.getById(11);

        t.chain(
            { drag : '[data-task-id="11"]', by : [100, 0], dragOnly : true },
            () => {
    ...
    t.it('Should fire correct events', async t => {
        ...
        gantt.on('beforetaskdropfinalize', ({ context }) => {
            const { taskRecords, valid } = context;
    ...
    t.it('Dragging w/ conflicts popup should not throw exception after browser looses focus', async t => {
        ...
        await t.waitForSelector('.b-schedulerpro-issueresolutionpopup header :contains(Scheduling conflict)');

        // emulate window loosing focus
    ...
    t.it('Should update pinSuccessors', async t => {
        ...
        // Set wrong name to disable the feature
        ...
        await t.dragBy({
            source  : '.b-gantt-task.id12',
            delta   : [tickSize, 0],
            options : { ctrlKey : true, shiftKey : true }
        });
    ...
    t.it('Should be able to block a drop by setting valid to false in beforetaskdropfinalize listener', async t => {
        ...
        await t.waitFor(() => {
            return dependencyPointsBeforeDrag.every((point, i) => {
                ...
                return Math.round(currentPoint.x) === Math.round(dependencyPointsBeforeDrag[i].x) &&
                    Math.round(currentPoint.y) === Math.round(dependencyPointsBeforeDrag[i].y);
            });
        });
