...
StartTest(t => {
                ...
                t.setIframe({
                  ...
                  onload(doc, frame) {
                    t.subTest(`Checking page ${i}`, async t => {
                      ...
                      const rows = doc.querySelectorAll('.b-timeline-subgrid .b-grid-row'),
  ...
  t.it('Should export visible schedule', async t => {
    ...
    gantt.zoomToSpan({
      ...
      centerDate
    }); // Wait till visible date range updated

    await gantt.await('visibleRangeChange', {
      checkLog: false
    });

    const expectedState = gantt.state,
          // this is precise visible range, we need to round it to ticks
    visibleRange = gantt.visibleDateRange,
  ...
  t.it('Should export large view', async t => {
    const {
      tasksData,
      dependenciesData
    } = await ProjectGenerator.generateAsync(500, 50, null, new Date(2020, 0, 1)),
    ...
    gantt = await t.getGanttAsync({
      ...
      features: {
        pdfExport: {
          ...
          headerTpl: ({
            currentPage
          }) => `<div style="height:61px;background-color: grey">
                    ${currentPage != null ? `Page ${currentPage}` : 'HEAD'}</div>`,
          footerTpl: () => `<div style="height:61px;background-color: grey">FOOT</div><style>.b-horizontaltimeaxis .b-sch-header-row { flex:1; }</style>`
        }
      },
      project: {
    ...
    await Promise.all([t.waitForSelector('.b-sch-dependency'), t.waitForSelector(gantt.unreleasedEventSelector)]);
