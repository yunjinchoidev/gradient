...
StartTest(t => {
  ...
  t.it('Should export large gantt to multiple pages', async t => {
    ...
    gantt = t.getGantt({
      ...
      features: {
        pdfExport: {
          ...
          headerTpl: ({
            currentPage
          }) => `<div style="height:9px;background-color: grey">
    ...
    await Promise.all([gantt.project.commitAsync(), t.waitForSelector('.b-sch-dependency')]);
    ...
    gantt = new Gantt({
      ...
      features: {
        pdfExport: {
          ...
          headerTpl: ({
            currentPage
          }) => `<div style="height:${headerHeight}px;background-color: grey">
                    ${currentPage != null ? `Page ${currentPage}` : 'HEAD'}</div>`,
      ...
      t.subTest(`Checking exported page ${i}`, async t => {
        ...
        const {
          taskStore
              ...
              tasks = taskStore.query(record => {
          return taskStore.indexOf(record) > rowsPerPage * (i % 3) - 1 && taskStore.indexOf(record) < rowsPerPage * (i % 3 + 1) - 1 && gantt.timeAxis.isTimeSpanInAxis(record);
        }),
              dependencies = gantt.dependencyStore.query(record => {
          return tasks.includes(record.fromEvent) && tasks.includes(record.toEvent);
        });
