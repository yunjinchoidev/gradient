...
StartTest(t => {
  ...
  t.it('Should export Gantt project to MSP XML format', async t => {
    ...
    const {
      taskStore
          ...
          task1 = taskStore.getById(1000); // update first task name w/ non ASCII symbols
    ...
    gantt.on({
      mspExport: ({
        ...
        t.is(xmlDoc.querySelector('Project>Name').textContent, 'FOO', 'project name is correct'); // sum + 1 to consider the project calendar (gantt.project.calendar) that is not present in the calendarManagerStore
  ...
  t.it('mspExport feature export() method uses proper way of encoding strings', async t => {
    ...
    const async = t.beginAsync(); // mock BrowserHelper.download used by the mspExport feature
