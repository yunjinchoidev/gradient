...
StartTest(t => {
    ...
    t.it('Should display only one arrow icon on parent node', async t => {
        ...
        t.chain(
            { drag : '.b-grid-cell:contains(Investigate)', to : '.b-grid-cell:contains(Report to management)', toOffset : [0, 2], dragOnly : true },
            { waitForSelector : '.b-row-reordering-target-parent[data-id="1"]', desc : 'Row reordering class showing the arrow on parent node "Planing"' },
            { mouseup : null },
            { waitForSelectorNotFound : '.b-row-reordering' },

            { drag : '.b-grid-cell:contains(Assign resources)', to : '.b-grid-cell:contains(Preparation work)', toOffset : [0, 2], dragOnly : true },
            { waitForSelector : '.b-row-reordering-target-parent[data-id="2"]', desc : 'Row reordering class showing the arrow on parent node "Implementation Phase"' },
            next => {
            ...
            { drag : '.b-grid-cell:contains(Choose technology suite)', to : '.b-grid-cell:contains(Step 1)', toOffset : [0, 2], dragOnly : true },
            { waitForSelector : '.b-row-reordering-target-parent[data-id="23"]', desc : 'Row reordering class showing the arrow on parent node "Build prototype"' },
            next => {
                t.selectorCountIs('.b-row-reordering-target-parent > .b-tree-cell', 1, 'Drag: There is just one task with reordering class applied showing the arrow');
                next();
            },
