import { ProjectModel, Rectangle } from '../../build/gantt.module.js?457330';
...
StartTest(t => {
    ...
    t.it('Should allow toggling indicators', async t => {
        ...
        t.chain(
            ...
            { waitForSelector : '.b-early-dates', desc : 'Early dates rendered' },
            { waitForSelectorNotFound : '.b-indicator .b-fa-dog', desc : 'Custom not rendered' }
        );
    });
    ...
    t.it('Should show point-in-time indicators that are inside time axis', async t => {
        ...
        t.chain(
            ...
            { waitForSelectorNotFound : gantt.unreleasedEventSelector + ' .b-task-percent-bar', desc : 'Task bar is not showing because is outside of the time axis.' },
            { waitForSelector : '.b-indicator .b-fa-beer', desc : 'Indicators inside axis rendered because is inside of the time axis' }
        );
    });
    ...
    t.it('Should show task bar inside time axis and hide indicator that is outside of the time axis', async t => {
        ...
        t.chain(
            ...
            { waitForSelectorNotFound : '.b-indicator .b-fa-dog', desc : 'Indicators is not rendered because is outside of the time axis' },
            { waitForSelector : '.b-task-percent-bar', desc : 'Task bar is showing because still is inside of the time axis.' }
        );
    });
