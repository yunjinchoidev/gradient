...
/* global ProjectModel */
StartTest(t => {
  const // get test descriptor (it has backend sync/load/reset URLs used in the test)
  ...
  let calendarManagerStore, resourceStore, assignmentStore, dependencyStore, taskStore;
  ...
  const task = name => findByName(taskStore, name),
        calendar = name => findByName(calendarManagerStore, name),
        resource = name => findByName(resourceStore, name);

  let project; // drop the database after the test finishes
  ...
  t.beforeEach((t, next) => {
    project = new ProjectModel({
      ...
      listeners: {
        loadfail: ({
          responseText
        }) => t.fail(`Loading failed (listener). responseText=${responseText}`)
      }
    });
    ({
    ...
    AjaxHelper.get(`${testConfig.resetUrl}?config=${configFile}`).catch(() => t.fail('Reset failed')).then(next);
  }, true);

  function noChangesAssertions(t) {
    t.notOk(project.crudStoreHasChanges(), 'No changes in project');
    t.notOk(calendarManagerStore.changes, 'No changes in calendarManagerStore');
    t.notOk(resourceStore.changes, 'No changes in resourceStore');
    t.notOk(assignmentStore.changes, 'No changes in assignmentStore');
    t.notOk(dependencyStore.changes, 'No changes in dependencyStore');
    t.notOk(taskStore.changes, 'No changes in taskStore');
  }

  t.it('Should be possible to save some resources, assignments, dependencies and tasks', t => {
    function checkProperties(obj, pattern, desc) {
      for (const key in Object.keys(pattern)) {
    ...
    t.chain(async () => {
      t.diag('Adding records'); // Finish delayed calculations

      await project.scheduleDelayedCalculation();
      general = calendarManagerStore.add({
        name: 'General'
      })[0];
      ...
      const [task1, task2, task3] = taskStore.add([Object.assign({
        name: 'task1',
      ...
      await project.commitAsync().catch(() => t.fail('Propagate failed'));
    }, {
      diag: 'Calling sync'
    }, async () => {
      ...
      await project.sync().catch(({
        responseText
      }) => t.fail(`Sync failed ${responseText}`));
    }, {
      ...
      t.diag('Calling load');
      await project.load().catch(({
        responseText
      }) => t.fail(`Load failed ${responseText}`));
    }, {
      diag: 'Checking records'
    }, async () => {
      noChangesAssertions(t);
      ...
      const task1 = task('task1'),
            ...
            // resource2 = resource('resource2'),
      resource3 = resource('resource3'); // add + edit + remove resources
      ...
      task1.constraintDate = new Date(2019, 0, 14);
      task1.manuallyScheduled = false;
      taskStore.remove(task3);
      await project.propagateAsync().catch(() => t.fail('Propagate failed'));
    }, async () => {
      t.diag('Calling sync');
      await project.sync().catch(({
        responseText
      ...
      checkProperties(task1, {
        ...
        constraintDate: new Date(2019, 0, 14),
  ...
  t.it('Should be possible to add a phantom task with a phantom sub-task', async t => {
    await project.scheduleDelayedCalculation();
    const [task1, task2] = taskStore.add([{
      name: '1'
    }, {
      name: '2'
    }]),
          task1IdBeforeSync = task1.id,
          task2IdBeforeSync = task2.id;
    task1.appendChild(task2);
    await project.propagateAsync().catch(() => t.fail('Propagate failed'));
    t.diag('Calling sync');
    await project.sync().catch(() => t.fail('Sync failed'));
    t.diag('Checking records');
    noChangesAssertions(t);
    t.notOk(resourceStore.count, 'no resources');
    t.notOk(dependencyStore.count, 'no dependencies');
    t.notOk(assignmentStore.count, 'no assignments');
    t.is(taskStore.allCount, 2, '2 tasks');
    t.isnt(task1.id, task1IdBeforeSync, 'task1 id updated');
    t.isnt(task2.id, task2IdBeforeSync, 'task2 id updated');
    t.notOk(task1.phantom, 'task1 is not phantom');
    t.notOk(task2.phantom, 'task2 is not phantom');
    t.diag('Calling load');
    await project.load().catch(({
      responseText
    }) => t.fail(`Load failed ${responseText}`));
  ...
  /*
      t.it('Prevents from persisting outdated data', function (t) {
          ...
          t.chain(
              ...
              function (next) {
                  ...
                  crud2.sync(function(){ t.fail('This sync should be failed'); next(); }, function() { t.pass('Sync successfully failed'); next(); });
              },
