...
StartTest(t => {
  ...
  t.it('Should send phantom parent id for added tasks', async t => {
    ...
    const [child21, child22] = newParent.children; // Child 1-2 is supposed to stretch parent

    await project.propagateAsync();
    let {
      tasks
    } = project.getChangeSetPackage();
    t.isDeeplySubset([{
      id: parent1.id,
    ...
    t.isDeeplySubset([{
      ...
      $PhantomParentId: newParent.id,
      parentId: newParent.id
    }], tasks.updated.sort((a, b) => a.id - b.id), 'Tasks update request is ok');
    t.isDeeplySubset([{
      $PhantomId: child12.id,
      parentId: parent1.id
    }, {
