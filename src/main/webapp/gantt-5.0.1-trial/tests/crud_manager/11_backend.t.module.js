import { AjaxHelper, TimeUnit, SchedulingMode, ConstraintType } from '../../build/gantt.module.js?457330';
...
StartTest(t => {
    ...
    t.it('Should be possible to save some resources, assignments, dependencies and tasks', t => {
        ...
        t.chain(
            async() => {
                ...
                // Finish delayed calculations
                await project.scheduleDelayedCalculation();

                general = calendarManagerStore.add({ name : 'General' })[0];

                subGeneral = general.appendChild({ name : 'Sub General' })[0];
    ...
    t.it('Should be possible to add a phantom task with a phantom sub-task', async t => {
        ...
        const
            [task1, task2]    = taskStore.add([{ name : '1' }, { name : '2' }]),
