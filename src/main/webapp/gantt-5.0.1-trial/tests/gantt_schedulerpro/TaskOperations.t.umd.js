...
StartTest(t => {
  ...
  t.beforeEach(() => {
    ...
    return (_container = container) === null || _container === void 0 ? void 0 : _container.destroy();
  });

  function createGanttAndSchedulerPro({
    partnered = true,
    ganttCfg = {},
    schedulerCfg = {}
  } = {}) {
  ...
  t.it('Should be possible to edit a task in Scheduler chart', t => {
    ...
    t.chain({
      ...
      doubleClick: '.b-schedulerpro [data-event-id="115"]',
      desc: 'Click on an event in Scheduler'
    }, {
      waitForSelector: '.b-gantttaskeditor',
      desc: 'Editor is shown'
    }, () => {
  ...
  t.it('Should be possible to edit a task in Gantt chart', t => {
    ...
    t.chain({
      ...
      doubleClick: '.b-gantt [data-task-id="120"]',
      desc: 'Click on a task in Gantt'
    }, {
      waitForSelector: '.b-gantttaskeditor',
      desc: 'Editor is shown'
    }, () => {
  ...
  t.it('Should be possible to edit a task with no resources in Gantt chart', t => {
    ...
    t.chain({
      ...
      doubleClick: '[data-task-id="9000"]',
      desc: 'Click on the task in Gantt'
    }, {
      waitForSelector: '.b-gantttaskeditor',
      desc: 'Editor is shown'
    });
  ...
  t.it('Should be possible to double click to create a task in Scheduler', t => {
    createGanttAndSchedulerPro();
    t.chain({
      doubleClick: '.b-schedulerpro .b-sch-timeaxis-cell',
      offset: [10, 10]
    }, {
      waitForSelector: '.b-gantttaskeditor',
      desc: 'Editor is shown'
      ...
      waitForSelector: '.b-schedulerpro .b-sch-event-wrap:contains(foo)',
  ...
  t.it('Should set correct startDate on dragCreate if gantt project used', async t => {
    createGanttAndSchedulerPro(); // Wait to make sure UI is not read-only
