...
Class('BryntumGanttTest', {
    ...
    methods : {
        async waitForPropagate(partOfProject, ...args) {
            const project = partOfProject.project || partOfProject;

            return this.waitFor(() => project.replica && !(project.replica.hasPendingAutoCommit() || project.replica.isCommitting), ...args);
        },
        ...
        getGantt(config = {}) {
            ...
            let projectConfig = {},
                project       = config.project?.construct ? config.project : null;
        ...
        async getGanttAsync(config) {
            const gantt = this.getGantt(config);
        ...
        async getRemoteGanttAsync(config = {}, delay = 300) {
            const
                me  = this,
            ...
            this.mockUrl(url, {
                responseText : JSON.stringify({
                    success : true,
                    tasks   : {
                        rows : me.getProjectTaskData()
                    },
                    dependencies : {
                        rows : me.getProjectDependenciesData()
                    },
                    calendars : {
                        rows : me.getProjectCalendarsData()
                    }
                }),
                parseJson : true,
                delay
            ...
            const gantt = me.getGantt(Object.assign(config, {
                project : new me.global.ProjectModel({
                    transport : {
                        load : { url }
                    }
                })
        ...
        forceTestVisible() {
            ...
            const grid = window.top.Ext.first('testgrid');
            grid.getSelectionModel().select(grid.store.getById(this.url));
        },

        verifyCachedDependenciesState(taskStore, dependencyStore) {
            ...
            dependencyStore.forEach(dependency => {
                const
                ...
                if (from && to) {
                    const
                        fromId   = from.id,
                        fromCopy = tasksCopies[fromId] = tasksCopies[fromId] || { outgoingDeps : new Set(), incomingDeps : new Set() };

                    fromCopy.outgoingDeps.add(dependency);

                    const
                        toId   = to.id,
                        toCopy = tasksCopies[toId] = tasksCopies[toId] || { outgoingDeps : new Set(), incomingDeps : new Set() };

                    toCopy.incomingDeps.add(dependency);
                }
            });

            let stateIsCorrect = true;

            taskStore.forEach(task => {
                const taskCopy = tasksCopies[task.id];
        ...
        assertDependency(gantt, dependency, { fromSide, toSide } = {}, msg = 'Assert dependency') {
            // getDependencyError returns an error string, so if it's falsy, it's success
            this.notOk(this.getDependencyError(gantt, dependency, { fromSide, toSide }, msg));
        },

        getDependencyError(gantt, dependency, { fromSide, toSide } = {}, msg = 'Assert dependency') {
            const
                me              = this,
                arrowMargin     = gantt.features.dependencies.pathFinder.startArrowMargin,
                getPointFromBox = (record, side) => {
                    const
                        adjustRight = 0;
                    let point,
                        adjustLeft = 0,
            ...
            if (from && to) {
                const
                    dependencyEl = gantt.features.dependencies.getElementForDependency(dependency),
                    fromPoint    = getPointFromBox(from, fromSide || getFromSide(dependency)),
                    toPoint      = getPointFromBox(to, toSide || getToSide(dependency));

                if (!dependencyEl) {
                    return `Dependency ${dependency.id} element not present`;
                }
                if (fromPoint && toPoint) {
                    const
                        ...
                        threshold        = this.bowser.safari ? 2 : 1.5;

                    this.isApproxPx(depFromPoint[0], fromPoint[0], threshold, `${msg}: Start point X is correct (${from.name})`);
                    this.isApproxPx(depFromPoint[1], fromPoint[1], threshold, `${msg}: Start point Y is correct (${from.name})`);

                    this.isApproxPx(depToPoint[0], toPoint[0], threshold, `${msg}: End point X is correct (${to.name})`);
                    this.isApproxPx(depToPoint[1], toPoint[1], threshold, `${msg}: End point Y is correct (${to.name})`);
                }
            }
        },
        ...
        async taskContextMenu(task, ...menuText) {
            const
                me        = this,
        ...
        async createGanttForExport({
            ...
            rowHeight = 50,
            rowsPerPage = 11,
            paperFormat = this.global.PaperFormat.A4,
            height = 450,
            width = 600,
            ...
            const
                timelineWeight                  = 0.75,
                paperHeight                     = paperFormat.height * 96,
                paperWidth                      = paperFormat.width * 96,
        ...
        assertExportedGanttDependenciesList(doc, dependencies = [], { fromSide, toSide } = {}, arrowMargin = 12) {
            ...
            const
                ...
                getPointFromBox = (doc, record, side, arrowMargin) => {
                    let point,
                        adjustLeft  = 0,
                        adjustRight = 0,
                        adjustTop   = 0,
                ...
                getScale        = el => el.getBoundingClientRect().width / parseFloat(el.style.width);
                    ...
                    if (dependencyEl) {
                        if (fromPoint && toPoint) {
                            const
                                svgBox           = dependencyEl.ownerSVGElement.getBoundingClientRect(),
                                dependencyPoints = dependencyEl.getAttribute('points').split(' '),
                                depStartPoint    = dependencyPoints[dependencyPoints.length - 1].split(',').map(item => parseFloat(item)),
                                depEndPoint      = dependencyPoints[0].split(',').map(item => parseFloat(item)),
                                ...
                                // We resolve wrapper element which is 1px wider than actual element we need
                                threshold        = bowser.firefox ? 5 : 3;
        ...
        assertExportedTasksList(doc, tasks = []) {
            ...
            tasks.forEach(task => {
                const taskElement = doc.querySelector(`.b-gantt-task-wrap[data-task-id="${task.id}"]`);
                ...
                if (!taskElement) {
                    ...
                    if (rowBox.intersect(taskBox, false, true).height !== taskBox.height) {
