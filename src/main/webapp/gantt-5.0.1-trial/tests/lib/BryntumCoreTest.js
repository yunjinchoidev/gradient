...
Class('BryntumCoreTest', {
    ...
    override : {
        ...
        verifyNoTimeoutsBeforeSubTest(test, callback) {
            const
                me           = this,
                globalDelays = me.global?.bryntum?.globalDelays;

            if (me.getConfig().disableNoTimeoutsCheck || !globalDelays || globalDelays.isEmpty()) {
                callback();
                return;
            }
            ...
            const
                ...
                MAX_POLL_COUNT = 50,
                poll           = () => {
                    if (globalDelays.isEmpty() || pollCount > MAX_POLL_COUNT) {
                        if (pollCount > MAX_POLL_COUNT) {
                            Object.entries(globalDelays).forEach(([k, v]) => {
                                if (v.size > 0) {
                                    const timoutNames = Array.from(v.values()).filter(v => v.name).map(v => v.name).join(', ');
                                    this.fail(`Found timeouts for "${k}" : ${timoutNames}`);
                                }
                            });
                        }
                        callback();
    ...
    methods : {
            ...
            if (!me.disableWaitingForCSSAnimations) {
                ...
                const hasNoAnimations = () => {
                    ...
                    return !me.global.document.getAnimations?.().some(a => {
                        ...
                        if (!el.matches(exemptAnimationTargets) && iterCount !== 'infinite') {
                            animatingEl = el;
                            return a;
                        }
                    });
        ...
        smartMonkeys(description) {
            ...
            const
                ...
                elements        = this.query(`.demo-header ${targetSelectors}, .b-toolbar ${targetSelectors}`);
        ...
        injectXSS(component) {
            const
                ...
                widgets = component ? [component] : [g.grid, g.scheduler, g.schedulerpro, g.gantt, g.taskboard, g.calendar];

            widgets.filter(widget => widget).forEach(widget =>
                ['columns', 'swimlanes', 'store', 'eventStore', 'taskStore', 'resourceStore'].forEach(storeName => {
                    const
                        record = widget.project?.[storeName]?.first || widget[storeName]?.first,
                    ...
                    if (fields) {
                        ...
                        ['text', 'name', 'city', 'email', 'description', 'status', 'prio', 'tags'].forEach(field => {
                            if (fields[field]) {
                                this.diag(`Inject XSS to ${widget.$$name}.${storeName}.${field}`);
        ...
        return items.filter(item => item).map(item => {
            ...
            if (cfg.pageUrl) {
                ...
                Object.assign(cfg, {
                    ...
                    keepPageUrl             : true,
                    name                    : `${monkeyName}-${id}`,
                    url                     : `examples/${monkeyName}.t.js?id=${id}&monkey&test`,
