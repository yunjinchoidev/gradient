import { PredecessorColumn, SuccessorColumn, TimeUnit, LocaleManager, DependencyField, ProjectModel } from '../../build/gantt.module.js?457330';
...
StartTest((t) => {
    ...
    t.it('Should use delimiter config for rendering and editing', async t => {
        gantt = await t.getGanttAsync({
            id      : 'gantt',
            columns : [
                { type : PredecessorColumn.type, delimiter : ',' },
                { type : SuccessorColumn.type, delimiter : '/' }
            ]
        });
    ...
    t.it('Should use dependencyIdField config for rendering and editing', async t => {
        gantt = await t.getGanttAsync({
            id      : 'gantt',
            columns : [
                { type : PredecessorColumn.type, dependencyIdField : 'wbsCode' },
                { type : SuccessorColumn.type,   dependencyIdField : 'wbsCode' }
            ]
        });
    ...
    t.it('Should update successors and predecessors properly on manual editing', t => {
        ...
        t.chain(
            ...
            { type : '12;13[ENTER]', clearExisting : true },
            { waitForSelector : '.id14 [data-column=predecessors]:contains(12;13)', desc : 'Data for predecessor has been changed properly' },
            { dblClick : '.id14 [data-column=successors]:contains(21;22)' },
            { type : '21[ENTER]', clearExisting : true },
            { waitForSelector : '.id14 [data-column=successors]:contains(21)', desc : 'Data for successor has been changed properly' }
        );
    });
    ...
    t.it('Should update existing successors when cell editing', async t => {
        ...
        // wait till changes get applied
        await t.waitFor(() => task.successors[0].lag === 2);

        const
            dep     = task.successors[0],
    ...
    t.it('Should update existing predecessors when cell editing', async t => {
        ...
        const
            dep     = gantt.taskStore.getById(21).predecessors[0],
            changes = gantt.dependencyStore.changes;
    ...
    t.it('Should pass validation after switch locale when cell editing', async t => {
        ...
        await t.doubleClick('.id21 [data-column=predecessors]:textEquals(14)');
        await t.type(null, '14EE[ENTER]', null, null, null, true);
    ...
    t.it('Should not show error message while typing, only when pressing ENTER', async t => {
        ...
        gantt.taskStore.add({ id : 5000, name : 'New task' });
    ...
    t.it('Should set lagUnit based on task durationUnit', async t => {
        gantt = t.getGantt({
            project : {
                taskStore : {
                    fields : [
                        { name : 'durationUnit', defaultValue : 'min' }
                    ]
                }
            },
    ...
    t.it('Should show children of collapsed tasks in the list', t => {
        ...
        gantt.collapseAll();
        gantt.taskStore.add({ name : 'new task', leaf : true });
        gantt.taskStore.add({ name : 'other task', leaf : true });
    ...
    t.it('Should add dependency if picker task list is filtered', async t => {
        ...
        gantt.dependencyStore.on('add', ({ records }) => {
    ...
    t.it('Filtering using context menu\'s "Equals"', async t => {
        ...
        t.selectorCountIs('.b-grid-subgrid-locked .b-grid-row', 6, 'Result: 3 leaves and 3 ancestors');
    ...
    t.it('Should change dependency type', async t => {
        ...
        const
            task         = gantt.taskStore.getById(21),
    ...
    t.it('Should show correct filter value after filtering using `equals` menu option', async t => {
        gantt = await t.getGanttAsync({
            ...
            columns : [
                { type : 'predecessor', width : 300, sortable : false }
            ]
        });
