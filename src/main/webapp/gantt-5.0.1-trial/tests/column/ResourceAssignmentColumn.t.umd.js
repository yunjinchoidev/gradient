...
StartTest(t => {
  ...
  t.it('Should render column properly', async t => {
    ...
    let raColumn,
        gotData = false;
    t.chain(async () => project.commitAsync(), {
  ...
  t.it('Editor should be configurable with floating assignments behavior', t => {
    function run(t, float) {
      ...
      t.chain(async () => project.commitAsync(), async () => {
  ...
  t.it('Should show tooltip when hovering avatar with failed loading of resource image', async t => {
    ...
    const avatarWithInitialsRect = t.rect('.b-resource-avatar-container .b-resource-avatar.b-resource-initials'),
  ...
  t.it('Should fire cell editing events for assignment column', async t => {
    ...
    t.chain({
      ...
      waitForSelector: '.b-assignmentfield .b-chip:contains(Secret Guy)',
      desc: 'Cell editing started in row below'
  ...
  t.it('Should support data mapping if cell editor used', async t => {
    class MockAssignmentModel extends AssignmentModel {
      static get fields() {
        return [{
          name: 'event',
          dataSource: 'custom_task'
        }, {
          name: 'resource',
          dataSource: 'custom_resource'
        }, {
          name: 'units',
          dataSource: 'custom_units',
          type: 'number'
        }];
      }
    ...
    t.chain({
      ...
      var _assignment$get, _assignment$get2;

      const assignment = gantt.project.assignmentStore.getAt(0);
      t.is(assignment.get('custom_units'), 11, 'Untis mapping ok');
      t.is((_assignment$get = assignment.get('custom_task')) === null || _assignment$get === void 0 ? void 0 : _assignment$get.id, 1, 'Event mapping ok');
      t.is((_assignment$get2 = assignment.get('custom_resource')) === null || _assignment$get2 === void 0 ? void 0 : _assignment$get2.id, 2, 'Resource mapping ok');
    });
  ...
  t.it('Should support filtering using context menu\'s Equals test', async t => {
    ...
    await t.click('[data-ref="saveBtn"]'); // Choose a task assined to Mike. There are two tasks assigned to Mike

    await t.rightClick('.id11 [data-column="assignments"]');
    await t.click('.b-menuitem[data-ref="filterStringEquals"]');
    t.selectorCountIs('.b-grid-subgrid-locked .b-grid-row', 6, 'Result: 2 leaves and 4 ancestors'); // Correct tasks visible

    t.is(gantt.taskStore.map(t => t.id).toString(), '1000,1,11,2,23,231');
    gantt.store.clearFilters(); // Choose an unassigned task for the "Equals" comparison.
    // This must filter to show all unassigned tasks.

    await t.rightClick('.b-grid-row.id22  [data-column="assignments"]');
    await t.click('.b-menuitem[data-ref="filterStringEquals"]');
    t.selectorCountIs('.b-grid-subgrid-locked .b-grid-row', 11, 'Result: 6 leaves and 5 ancestors'); // Correct tasks visible
