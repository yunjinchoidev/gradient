...
StartTest(t => {
    ...
    t.it('Check all extra available columns', async t => {
        ...
        t.isDeeply(addNewColumn.combo.picker.store.records.map(r => ({ id : r.id, text : r.text })),
            [
    ...
    t.it('Create new column', async t => {
        const
            newColumnsStore = addNewColumn.combo.store,
        ...
        await t.waitFor(() => addNewColumn.combo._picker?.isVisible);
