  ...
  t.it('Should localize AddNewColumn dynamically', t => {
    ...
    for (const name of Object.keys(window.bryntum.locales)) {
      t.describe(`${name} locale is ok`, async t => {
        ...
        t.isDeeply(combo.picker.store.records.map(r => ({
          id: r.id,
          text: r.text
        })), [{
          id: 'percentdone',
          ...
          id: 'inactive',
          text: locale.InactiveColumn.Inactive
        }, {
          id: 'lateenddate',
        ...
        }].sort((a, b) => a.text < b.text ? -1 : 1), 'Combobox has all the columns translated and sorted');
      });
    }
  });
});