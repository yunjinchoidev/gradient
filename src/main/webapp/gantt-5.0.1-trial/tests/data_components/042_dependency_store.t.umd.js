...
function getDataSet(dependencies, tasks, taskStoreCfg, depStoreCfg) {
  const dependencyStore = new DependencyStore(Object.assign({
    data: dependencies || [{
      id: 123,
      fromEvent: 1,
      toEvent: 2,
...
StartTest(t => {
  t.it('Basic functionality', async t => {
    ...
    await dep.propagateAsync();
    t.verifyCachedDependenciesState(taskStore);
    dep.toEvent = newTask;
    await dep.propagateAsync(); // SHOULD CLEAR THE CACHE SOMEWHERE?
    ...
    var cacheKey = dependencyStore.buildCacheKey(1, 2, null, null, {
      visitedTasks: {}
    });
