...
StartTest(t => {
  ...
  t.it('Should be possible to add a copy of an unscheduled task', async t => {
    ...
    const original = project.taskStore.rootNode.appendChild({
      name: 'New'
    }),
  ...
  t.it('Task baselines', async t => {
    ...
    for (const task of eventStore) {
      const b0 = task.baselines.first; // Task id 3 has slipped by one day from its baseline end date of
      // 21 Jul and 16 days.
  ...
  t.it('should send all fields if writeAllFields is true', async t => {
    ...
    const pack = project.getChangeSetPackage(),
          taskDataToBeSent = pack.tasks.updated[0];
    t.is(pack.tasks.updated.length, 1, 'One task changed'); // When testing on different machines it might turn out that current date and task start are in different time zone offset
  ...
  t.it('Should trigger schedulingconflict event without errors', async t => {
    ...
    project.on('schedulingconflict', context => {
      context.continueWithResolutionResult( // originally bryntum.gantt.EffectResolutionResult.Cancel should be used
