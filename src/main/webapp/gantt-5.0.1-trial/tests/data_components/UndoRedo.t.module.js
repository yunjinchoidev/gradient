import { ConstraintType, Toast, BrowserHelper } from '../../build/gantt.module.js?457330';
...
StartTest(t => {
    ...
    t.it('Should keep recording changes if project is committing', async t => {
        ...
        // set smallest possible timeout for autoRecording state to make sure it finished before project is recalculated
        stm.autoRecordTransactionStopTimeout = 1;

        // append task to start project recalculation
        gantt.taskStore.rootNode.appendChild({ name : 'New task', duration : 30 });

        await gantt.project.await('dataReady', false);

        // Check if after a timeout no new items triggered by the autoRecording state have appeared in the queue
    ...
    t.it('Should not record changes to purely calculated fields like `earlyStart`, `critical` etc', async t => {
        ...
        // This does change any ´real´ data, just calculates a few fields so it should not trigger a recording
    ...
    t.it('Should react to CTRL-Z when configured with enableUndoRedoKeys', async t => {
        ...
        t.is(gantt.project.changes, null, 'Undid changes');

        // REDO
        await t.type(null, 'Z', null, null, { shiftKey : true, ctrlKey : !BrowserHelper.isMac, metaKey : BrowserHelper.isMac });
        await t.waitFor(() => stm.canUndo);
