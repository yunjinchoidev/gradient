...
describe(t => {
  let consoleMessages, consoleSpy, project;
  ...
  t.it('Setting and getting inlineData', async t => {
    ...
    project = new ProjectModel(); // Populate the stores

    project.inlineData = testData; // Get data back

    const inlineData = project.inlineData; // Check data
  ...
  t.it('Should warn when load response format is incorrect', async t => {
    ...
    await project.load();
    t.like(consoleMessages.shift(), buildLinesRegExp(['Project load response error(s):', '- No "tasks" store section found. It should contain the store data.', 'Please adjust your response to look like this:', '{', '    "tasks": {', '        "rows": [', '            ...', '        ]', '    }', '}', 'Note: To disable this validation please set the "validateResponse" config to false']), 'Correct warn message shown.');
    t.mockUrl('load', {
      responseText: JSON.stringify({
        assignments: {
          rows: [{
    ...
    await project.load();
    t.like(consoleMessages.shift(), buildLinesRegExp(['Project load response error(s):', '- "tasks" store section should have a "rows" property with an array of the store records.', 'Please adjust your response to look like this:', '{', '    "tasks": {', '        "rows": [', '            ...', '        ]', '    }', '}', 'Note: To disable this validation please set the "validateResponse" config to false']), 'Correct warn message shown.');
    t.mockUrl('load', {
      responseText: JSON.stringify({
        assignments: {
          rows: [{
  ...
  t.it('Should warn when sync response format is incorrect', async t => {
    ...
    await project.sync();
    t.like(consoleMessages.shift(), buildLinesRegExp(['Project sync response error(s):', '- "tasks" store section should be an Object.', 'Please adjust your response to look like this:', '{', '    "tasks": {', '        ,,,', '    }', '}', 'Note: To disable this validation please set the "validateResponse" config to false']), 'Correct warn message shown.');
    t.mockUrl('sync', {
      responseText: JSON.stringify({
        success: true,
        type: 'sync',
    ...
    taskStore.add({});
    await project.sync();
    t.like(consoleMessages.shift(), buildLinesRegExp(['Project sync response error(s):', '- "tasks" store "rows" section should mention added record(s) #XXX sent in the request. It should contain the added records identifiers (both phantom and "real" ones assigned by the backend).', 'Please adjust your response to look like this:', '{', '    "tasks": {', '        "rows": [', '            {', '                "$PhantomId": XXX,', '                "id": ...', '            }', '        ]', '    }', '}', 'Note: To disable this validation please set the "validateResponse" config to false']), 'Correct warn message shown.');
  }); // https://github.com/bryntum/support/issues/2668
  ...
  t.it('Should warn when sync response format is incorrect and supportShortSyncResponse is false', async t => {
    ...
    taskStore.first.name = 'bar';
    await project.sync();
    t.like(consoleMessages.shift(), buildLinesRegExp(['Project sync response error(s):', '- "tasks" store "rows" section should mention updated record(s) #XXX sent in the request. It should contain the updated record identifiers.', 'Please adjust your response to look like this:', '{', '    "tasks": {', '        "rows": [', '            {', '                "id": XXX', '            },', '            ...', '        ]', '    }', '}', 'Note: Please consider enabling "supportShortSyncResponse" option to allow less detailed sync responses (https://bryntum.com/docs/gantt/api/Gantt/model/ProjectModel#config-supportShortSyncResponse)', 'Note: To disable this validation please set the "validateResponse" config to false']), 'Correct warn message shown.');
    project.revertChanges();
    t.mockUrl('sync', {
      responseText: JSON.stringify({
        success: true,
    ...
    await project.sync();
    t.like(consoleMessages.shift(), buildLinesRegExp(['Project sync response error(s):', '- "tasks" store "removed" section should mention removed record(s) #XXX sent in the request. It should contain the removed record identifiers.', 'Please adjust your response to look like this:', '{', '    "tasks": {', '        "removed": [', '            {', '                "id": XXX', '            },', '            ...', '        ]', '    }', '}', 'Note: Please consider enabling "supportShortSyncResponse" option to allow less detailed sync responses (https://bryntum.com/docs/gantt/api/Gantt/model/ProjectModel#config-supportShortSyncResponse)', 'Note: To disable this validation please set the "validateResponse" config to false']), 'Correct warn message shown.');
  }); // https://github.com/bryntum/support/issues/2668
  ...
  t.it('Should relay store changes', async t => {
    ...
    await project.commitAsync();
    t.isDeeply(changeData, {
      store: project.taskStore,
      action: 'update'
    }, 'Correct args for TaskStore update'); //
    // RESOURCESTORE
    //
    ...
    t.isDeeply(changeData, {
      ...
      action: 'update'
    }, 'Correct args for ResourceStore update'); //
    // ASSIGNMENTSTORE
    //
    ...
    await project.commitAsync();
    t.isDeeply(changeData, {
      store: project.assignmentStore,
      action: 'update'
    }, 'Correct args for AssignmentStore update'); //
    // DEPENDENCYSTORE
    //
    ...
    await project.commitAsync();
    t.isDeeply(changeData, {
      store: project.dependencyStore,
      action: 'update'
    }, 'Correct args for DependencyStore update'); //
    // TIMERANGESTORE
    //
    ...
    t.isDeeply(changeData, {
      ...
      action: 'update'
    }, 'Correct args for TimeRangeStore update');
  ...
  t.it('Should apply proper calendar when loading other data to the project', async t => {
    var _project$calendar, _project$calendar2;
    ...
    t.is((_project$calendar = project.calendar) === null || _project$calendar === void 0 ? void 0 : _project$calendar.id, 'a', 'project calendar is correct');
    ...
    t.is((_project$calendar2 = project.calendar) === null || _project$calendar2 === void 0 ? void 0 : _project$calendar2.id, 'b', 'project calendar is correct');
