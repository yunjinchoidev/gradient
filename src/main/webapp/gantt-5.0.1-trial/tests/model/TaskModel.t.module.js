import { Store, TaskModel, ProjectModel } from '../../build/gantt.module.js?457330';
...
StartTest(t => {
    ...
    t.it('Should calculate baselines dates and durations correctly', async t => {
        ...
        t.diag('Standalone task');

        // Task not included in project
        ...
        t.is(baselines[2].startDate, new Date('2021-05-22T00:00:00'), 'Correct baselines[2] startDate');
        ...
        // Task in project - uses calendar for calculations
    ...
    t.it('Should mark calculated fields as ignored', async t => {
        ...
        TaskModel.addField({ name : 'foo', persist : false });
    ...
    t.it('Should not report changes existing after filtering', async t => {
        ...
        t.isDeeply(store.changes.modified[0].modificationData, {
            id : 1, name : 'foo', parentIndex : 1
        }, 'Correct #1 change reported');
        t.isDeeply(store.changes.modified[1].modificationData, { id : 3, parentIndex : 0 }, 'Correct #3 change reported');
        t.isDeeply(store.changes.modified[2].modificationData, { id : 2, parentIndex : 2 }, 'Correct #2 change reported');
    ...
    t.it('Setting duration on detached event in other store should trigger change', async t => {
        ...
        task.fullDuration = { magnitude : 2, unit : 'days' };
    ...
    t.it('Should move task pinning dependent tasks by dependency lag', async t => {
        const project = new ProjectModel({
            ...
            dependenciesData : [
                ...
                { id : 3, from : 1, to : 4, type : 2 },
                { id : 4, from : 1, to : 5, type : 3 },

                { id : 5, from : 1, to : 6, type : 0 },
                { id : 6, from : 1, to : 7, type : 1 },
                { id : 7, from : 1, to : 8, type : 2 },
                { id : 8, from : 1, to : 9, type : 3 }
            ]
        });
