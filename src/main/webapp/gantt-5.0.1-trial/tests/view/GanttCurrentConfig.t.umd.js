...
StartTest(t => {
  ...
  t.it('Should get (mostly) empty app with empty config', t => {
    gantt = new Gantt();
    const str = gantt.getTestCase({
      output: 'return'
    });
    t.is(str, `import("../../build/gantt.module.js").then(module => { Object.assign(window, module);\n\nconsole.log('Gantt ${VersionHelper.getVersion('gantt')}');\n      \nconst gantt = new Gantt({\n  features: {\n    group: false,\n    tree: true,\n    regionResize: true\n  }\n});\n});`, 'Correct app');
  });
  t.it('Should inline data on project', t => {
    ...
    const str = gantt.getConfigString(); // eslint-disable-next-line no-new-func
