...
StartTest(t => {
  ...
  t.it('Sanity', async t => {
    ...
    t.selectorExists('.b-gantt-task', 'Task rendered'); // assert enabled features
  ...
  t.it('Should take an later-than-display-range visibleDate into account when choosing date range for time axis', async t => {
    ...
    await t.waitFor(() => {
      return ganttBase.startDate.getTime() === new Date(2017, 0, 15).getTime() && ganttBase.endDate.getTime() === new Date(2018, 4, 6).getTime() && ganttBase.visibleDateRange.startDate.getTime() < new Date(2018, 4, 2).getTime() && ganttBase.visibleDateRange.endDate.getTime() > new Date(2018, 4, 2).getTime();
    });
    const range = ganttBase.visibleDateRange;
