...
StartTest(t => {
  ...
  t.it('should render tasks on demand during scroll', async t => {
    gantt = t.getGantt({
      height: 150 // Used to be 100 but did not fit a full row and scroller then refused to scroll

    });
    await gantt.project.await('dataReady'); // Last task's element is NOT present because of buffered rendering.
    ...
    () => gantt.scrollTaskIntoView(gantt.store.last), // Wait for last task's element to be present.
    {
      waitFor: () => gantt.getElementFromTaskRecord(gantt.store.last)
    });
  });
  t.it('task navigation', async t => {
    gantt = t.getGantt({
      height: 700,
    ...
    const editTaskSpy = t.spyOn(gantt.features.taskEdit, 'editTask').and.callFake(() => {}),
          contextmenuSpy = t.spyOn(gantt.features.taskMenu, 'internalShowContextMenu').and.callFake(() => {}),
          testChain = [{
    ...
    let counter = 0; // Navigate through the entire chart, checking triggering of keyboard-based
    // operations on each Task

    gantt.taskStore.forEach(task => {
      const taskEl = gantt.getElementFromTaskRecord(task, false),
            callCount = ++counter;
      testChain.push({
        waitFor: () => document.activeElement === taskEl,
        desc: 'Task element is active'
      }, {
        waitFor: () => visitedTasks[callCount - 1] === taskEl,
        decs: 'Task element is correct'
      }, {
        type: '[ENTER]'
  ...
  t.it('Should hide task elements when none are visible in the time span', t => {
    ...
    const taskSelector = gantt.eventSelector,
  ...
  t.it('task navigation with Gantt inside a Panel', async t => {
    gantt = t.getGantt({
      onTaskBarClick: () => {},
      height: 600,
      appendTo: null
    ...
    gantt.taskStore.forEach(task => {
      const innerEl = gantt.getElementFromTaskRecord(task),
  ...
  t.it('Adding new records', async t => {
    gantt = t.getGantt({
    ...
    panel.bodyElement.style.padding = 0;
    let cellEdit, cellEditor;
    const count = gantt.taskStore.count;
    t.chain({
      click: panel.widgetMap.addTaskButton.element
  ...
  t.it('Dates are displayed consistently', t => {
    ...
    function assertTask(name) {
      ...
      {
        dblclick: `.b-gantt-task.${name}`,
        desc: 'Checking event editor'
      }, {
        waitForSelector: '.b-gantt-taskeditor'
      }, next => {
        t.is(document.querySelector('.b-gantt-taskeditor input[name=startDate]').value, startDateStr, `${name} start date field in event editor is ok`);
        t.is(document.querySelector('.b-gantt-taskeditor input[name=endDate]').value, endDateStr, `${name} end date field in event editor is ok`);
        next();
      }, {
        click: '.b-popup-close'
      }];
  ...
  t.it('ESC handling should not proceed as if a cell was focused if a Task is focused', t => {
    ...
    const node1 = gantt.taskStore.getById(1); // The correct outcome here is that it does NOT throw an error.
    ...
    t.chain({
      ...
      type: '[ESCAPE]'
    }, // We are waiting for nothing to happen. The error was that ESC attempted to shift focus
    {
      waitFor: 100
    }, () => {
      // Focus must not have been moved
      t.is(gantt.activeEvent, node1, 'Active event is correct');
    });
  });
  t.it('Should not duplicate task on changing id', async t => {
    gantt = t.getGantt({
      appendTo: document.body,
      tasks: [{
        id: 1,
    ...
    await gantt.project.await('dataReady');
    t.selectorCountIs(gantt.unreleasedEventSelector, 1, 'Only one task is rendered');
  ...
  t.it('should support scrolling unscheduled task into view', t => {
    ...
    const task = gantt.taskStore.rootNode.appendChild({
      cls: 'foo'
    });
    t.chain( // Scroll to end
    () => gantt.scrollTaskIntoView(task), // Wait for task element to be present.
    {
  ...
  }); // do not run performance test for UMD bundle

  !t.url.match(/.umd./) && t.it('Should have sane performance when replacing dataset', async t => {
    gantt = await t.getGanttAsync({
      project: await ProjectGenerator.generateAsync(1000, 50, null, undefined, false)
    });
    ...
    t.diag(`${timeTaken}ms`);
    t.isLess(timeTaken, 1000, 'Reasonable fast');
  });
  t.it('Gantt destruction must destroy all descendants', async t => {
    gantt = t.getGantt({
      onTaskBarClick: () => {},
      appendTo: document.body,
      height: 600,
    ...
    await gantt.project.await('dataReady');
    const allWidgets = []; // Gather all descendant widgets

    gantt.eachWidget(w => allWidgets.push(w), true);
    gantt.destroy(); // All descendant widgets MUST be destroyed.

    for (let i = 0, {
      length
    } = allWidgets; i < length; i++) {
      t.ok(allWidgets[i].isDestroyed);
    }
  }); // https://github.com/bryntum/support/issues/2380
  // https://github.com/bryntum/support/issues/4033

  t.it('Gantt should trigger DOM events', async t => {
    gantt = await t.getGanttAsync();
    const record = gantt.eventStore.getAt(4),
          taskEl = gantt.getElementFromEventRecord(gantt.eventStore.getAt(4));
    ['taskClick', 'taskKeyDown', 'taskKeyUp', 'taskMouseOver', 'taskMouseOut', 'taskMouseUp', 'taskMouseDown'].forEach(evName => {
      t.firesAtLeastNTimes(gantt, evName, 1);

      if (evName === 'taskKeyDown' || evName === 'taskKeyUp') {
        t.isFiredWithSignature(gantt, evName, ({
          source,
          taskRecord,
          event
        }) => {
          t.is(source, gantt, 'source');
          t.is(taskRecord, record, 'taskRecord');
          t.isInstanceOf(event, KeyboardEvent, 'event');
          return true;
        });
      }
    });
    await t.click(taskEl);
    await t.type(null, 'A');
    await t.moveCursorTo({
      target: '.b-sch-header-timeaxis-cell',
      offset: [5, 5]
    });
  });
});