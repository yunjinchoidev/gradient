import { ObjectHelper, Wbs, ProjectModel, TaskModel } from '../../build/gantt.module.js?457330';
...
StartTest(t => {
    ...
    function sortByWbs(ascending = true) {
        gantt.taskStore.addSorter({ field : 'wbsValue', ascending });
    }

    async function getGantt(ganttConfig, eventsData) {
        gantt = await t.getGanttAsync(ObjectHelper.merge({
            height    : 700,
            startDate : ganttConfig?.project?.startDate ?? '2017-06-19',
            columns   : [{ type : 'wbs' }],
            project   : {
    ...
    t.it('should reassign WBS values when setting inlineData', async t => {
        ...
        t.isDeeply(getItems(), [
            '> Herp (1)',
            '>> Derp (1.1)'
        ], 'Correct after assigning inlineData');
    });
    ...
    t.it('should maintain WBS values when reverse sorted', async t => {
        ...
        t.isDeeply(getItems(), [
            ...
            '>>> Node 3 (2.1.3)',
            '>> Node 1 (2.2)'
        ], 'Correct after clear sorters and refresh');
    });

    t.it('should maintain WBS values when sorted and filtered', async t => {
        await getGantt();

        // await t.click('[data-column="wbsCode"]');
        sortByWbs(false);
    ...
    t.it('should auto update WBS values when sorted and inserting child using wbsMode:auto', async t => {
        ...
        t.expect(getItems(project.taskStore)).toEqual([
            '> b (1)',
            '> a (2)'
        ]);

        // add a child to task 'b' (which is at the top of the list bc of the sort but currently has a wbs value of 2)
        taskStore.getById(29).insertChild({ id : 30, name : 'c' });
    ...
    t.it('should track persistable WBS values when inserting items', async t => {
        ...
        t.isDeeply(
            gantt.taskStore.added.map(({ persistableData }) => [persistableData.id, persistableData.wbsValue]),
            [
                [200, '1.2']
            ],
            'the added record wbsValue is included into the data for persisting'
        );

        t.isDeeply(
            gantt.taskStore.modified.map(({ modificationDataToWrite }) => modificationDataToWrite),
            [
                { id : 2, wbsValue : '1.3', parentIndex : 2 },
                { id : 3, wbsValue : '1.3.1' },
                { id : 4, wbsValue : '1.3.2' },
                { id : 5, wbsValue : '1.3.3' }
            ],
