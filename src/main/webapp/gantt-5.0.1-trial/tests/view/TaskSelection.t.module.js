import { ProjectGenerator } from '../../build/gantt.module.js?457330';
    ...
    t.it('Should clear selection classes when scrolling the view', async t => {
        gantt = await t.getGanttAsync({
            ...
            tbar : [{
                ...
                text : 'scroll',
                onClick() {
                    const { rows } = gantt.rowManager;

                    return gantt.scrollRowIntoView(rows[rows.length - 1].id, { block : 'start' });
                }
            }]
        });
        ...
        await gantt.scrollRowIntoView(rows[rows.length - 1].id, { block : 'start' });
