import { Gantt, CSSHelper, Rectangle, ProjectModel } from '../../build/gantt.module.js?457330';
...
StartTest(t => {
    ...
    function isTaskPositioned(t, taskOrId) {
        ...
        return taskBox.top >= rowBox.top && taskBox.bottom <= rowBox.bottom &&
            (task.isMilestone || t.samePx(taskLeft, gantt.getCoordinateFromDate(task.startDate, true)));
    }
    ...
    t.it('Gantt should refresh on task add/update/remove', async t => {
        ...
        t.chain(
            ...
            { waitFor : () => tasksPositioned(t), desc : 'Tasks positioned ok' },
    ...
    t.it('Should add task below', t => {
        ...
        t.chain(
            ...
            async() => {
                t.isDeeply(
                    Array.from(document.querySelectorAll('.b-grid-cell[data-column=wbsValue]')).map(el => el.textContent),
                    ['1', '1.1', '1.2', '2', '3']
                );
    ...
    t.it('Should add task below a task with constraint', async t => {
        ...
        // contraint shouldn't be copied, task should starts from project's startDate
    ...
    t.it('Should trigger renderTask and releaseTask', async t => {
        ...
        // +1 since Project A is first rendered with unnormalized dates, then rerendered with correct dates
