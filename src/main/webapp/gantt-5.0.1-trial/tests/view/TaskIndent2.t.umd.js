...
StartTest(t => {
  ...
  t.it('Sanity checks', async t => {
    ...
    {
      waitFor: () => !node2.isLeaf && node2.startDate.valueOf() === new Date(2010, 0, 1).valueOf() && node2.endDate.valueOf() === new Date(2010, 0, 3).valueOf()
    }, next => {
    ...
    {
      waitFor: () => node4.parent === node2 && node2.startDate.valueOf() === new Date(2010, 0, 1).valueOf() && node2.endDate.valueOf() === new Date(2010, 0, 7).valueOf()
    }, () => {
      t.is(node3.startDate, new Date(2010, 0, 1), 'Node3 start');
      t.is(node3.endDate, new Date(2010, 0, 3), 'Node3 end');
  ...
  t.it('Indent non-indentable node using UI must be no-op', async t => {
    const taskStore = getGantt({
      project: {
        startDate: '2010-01-01',
        eventsData: [{
  ...
  t.it('Indent non-indentable node using API must be no-op', async t => {
    const taskStore = getGantt({
      project: {
        startDate: '2010-01-01',
        eventsData: [{
    ...
    t.chain({
      waitForPropagate: project
    }, () => gantt.indent([t2]), // We are waiting for something *NOT* to happen. There should be no propagate
    // so we can't wait for it.
    {
      waitFor: 100
    }, () => {
  ...
  t.it('Indent with dependencies linking a child-parent after move using UI must be reverted', async t => {
    const taskStore = getGantt({
      project: {
        startDate: '2010-01-01',
        eventsData: [{
    ...
    const t1 = taskStore.getById(1),
          t3 = taskStore.getById(3);
    t.chain({
      waitForPropagate: project
    }, async () => t.taskContextMenu(t3, 'Indent'), {
  ...
  t.it('Indent with dependencies linking a child-parent after move using API must be reverted', async t => {
    const taskStore = getGantt({
      project: {
        startDate: '2010-01-01',
        eventsData: [{
    ...
    t.chain({
      ...
      // We intentionally do not return indent() result (which is a Promise)
      // since the Promise gets resolved only after we click Cancel button on the next step
      gantt.indent([t3]);
    }, {
      click: 'label:textEquals(Cancel)'
    }, {
  ...
  t.it('Indent with dependencies where a cycle is created using UI must be reverted.', async t => {
    const taskStore = getGantt({
      project: {
        startDate: '2010-01-01',
        eventsData: [{
  ...
  t.it('Indent with dependencies where a cycle is created using API must be reverted.', async t => {
    const taskStore = getGantt({
      project: {
        startDate: '2010-01-01',
        eventsData: [{
