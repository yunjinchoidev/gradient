...
StartTest(t => {
        ...
        gantt = await t.getGanttAsync({
            ...
            columns : [
                { type : 'name' },
                {
                    type       : 'duration',
                    filterable : {
                        filterField : { type : 'durationfield' },
                        filterFn    : ({ record, value, operator }) => {
                            switch (operator) {
                                case '<': return record.fullDuration < value;
                                case '>': return record.fullDuration > value;
                                case '=': return record.fullDuration.valueOf() === value.valueOf();
                            }
                        }
                    }
                }
    ...
    t.it('Sorting should first be applied after initial commit (restoring state)', async t => {
        ...
        gantt.taskStore.sort({ field : 'startDate', ascending : false });
        ...
        // do not wait for project to be ready on purpose
        gantt = t.getGantt({
            project : {
                startDate : '2020-11-30',
                eventsData
        ...
        // Wait for project to calculate
        await gantt.project.await('dataReady');

        // Store state will be restored in the next animation frame after dataReady, so we need to wait for one
        await t.waitForAnimationFrame();

        t.isDeeply(gantt.taskStore.sorters?.[0], { field : 'startDate', ascending : false }, 'Sorter is applied');
        t.isDeeply(gantt.taskStore.getRange().map(r => r.id), [1, 2, 3], 'Task order is correct');
    });
    ...
    t.it('Filters should first be applied after initial commit (restoring state)', async t => {
        ...
        // Sort tasks in reverse order by start date. They won't change order
        gantt.taskStore.filter({ property : 'endDate', operator : '<', value : new Date('2020-12-04') });
    ...
    t.it('Expand trigger should be visible after apply collapsed state for normal grid', async t => {
        ...
        ({ normal } = gantt.subGrids);

        // If both locked and normal grid width is defined it wouldn't allow proper resizing using splitter.
        ...
        t.is(gantt.state.subGrids.normal.width, undefined, 'State should not define width of the normal grid');
