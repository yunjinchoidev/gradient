import { StateProvider, Gantt } from '../../../build/gantt.module.js?457330';
...
StartTest(t => {
    ...
    const
        getStateSpy = t.spyOn(Gantt.prototype, 'getState'),
    ...
    t.it('Should store state in the background (inline data)', async t => {
        const
            ...
            ganttConfig   = {
                ...
                columns : [
                    { id : 'name', type : 'name', width : 150 },
                    { id : 'timeAxis', type : 'timeAxis' }
                ]
            };
        ...
        // Sorting should trigger state
        await runStep(() => gantt.zoomIn(), 'Zoom in');

        // Tick size should trigger state
        await runStep(() => gantt.tickSize += 10, 'Tick size');

        // Bar margin
        await runStep(() => gantt.barMargin += 2, 'Bar margin');
        ...
        // New gantt should apply state from start
        ...
        // Assert center date when this issue is fixed
    ...
    t.it('Should apply state on load', async t => {
        ...
        await Promise.all([
            t.click('.id11.b-gantt-task'),
            gantt.await('selectionchange', false)
        ]);

        await t.waitFor({
            method : () => JSON.parse(localStorage.getItem(`bryntum-state:${STATE_KEY}`))?.selectedCell,
            desc   : 'State is saved'
        });
    ...
    t.it('Should sync state after reconfiguring the project', async t => {
        ...
        t.mockUrl('load', {
            responseText : JSON.stringify({
                ...
                tasks   : {
                    rows : [
                        { id : 1, name : 'Task 1', startDate : '2017-01-15', duration : 1 },
        ...
        gantt.project.taskStore.sort({ field : 'name', ascending : false });
