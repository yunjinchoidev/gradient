import { Panel, AllColumns, ProjectModel, DateHelper, VersionHelper, ProjectGenerator } from '../../build/gantt.module.js?457330';
...
StartTest(t => {
    ...
    t.it('task navigation', async t => {
        ...
        gantt.taskStore.forEach(task => {
            const
                taskEl    = gantt.getElementFromTaskRecord(task, false),
                callCount = ++counter;

            testChain.push(
                { waitFor : () => document.activeElement === taskEl, desc : 'Task element is active' },
                { waitFor : () => visitedTasks[callCount - 1] === taskEl, decs : 'Task element is correct' },
                { type : '[ENTER]' },
    ...
    !t.url.match(/.umd./) && t.it('Should have sane performance when replacing dataset', async t => {
        ...
        gantt.taskStore.data = [{ id : 1, startDate : '2017-01-20', duration : 1 }];
    ...
    t.it('Gantt destruction must destroy all descendants', async t => {
        ...
        // Gather all descendant widgets
        gantt.eachWidget(w => allWidgets.push(w), true);

        gantt.destroy();

        // All descendant widgets MUST be destroyed.
        for (let i = 0, { length } = allWidgets; i < length; i++) {
    ...
    t.it('Gantt should trigger DOM events', async t => {
        ...
        const
            record = gantt.eventStore.getAt(4),
            taskEl = gantt.getElementFromEventRecord(gantt.eventStore.getAt(4));

        ['taskClick', 'taskKeyDown', 'taskKeyUp', 'taskMouseOver', 'taskMouseOut', 'taskMouseUp', 'taskMouseDown'].forEach(
            evName => {
                t.firesAtLeastNTimes(gantt, evName, 1);

                if (evName === 'taskKeyDown' || evName === 'taskKeyUp') {
                    t.isFiredWithSignature(gantt, evName, ({ source, taskRecord, event }) => {
