import { SchedulerProCycleEffect, DeactivateDependencyResolution, RemoveDependencyResolution, EffectResolutionResult, Gantt } from '../../build/gantt.module.js?457330';
...
StartTest(t => {
    ...
    t.it('Should resolve a nested parent-child cycle', async t => {
        ...
        t.it('Resolve by deactivating dependencies one by one', async t => {
            ...
            const { eventStore, dependencyStore } = project;

            let eventFired = 0;

            project.on(
                'cycle',
                async({ schedulingIssue, continueWithResolutionResult }) => {
                    ...
                    const expectedDependencies = [[dep2], [dep1]];

                    // debugger

                    t.diag(`${eventFired + 1} schedulingIssue caught`);

                    t.isInstanceOf(schedulingIssue, SchedulerProCycleEffect, 'proper schedulingIssue found');

                    // assert arrays content ignoring sorting order
            ...
            await t.click('label:contains(Deactivate)');

            await t.click('label:textEquals(Apply)');

            await t.waitForSelector('.b-error-description:contains("Launch SaaS Product")');
