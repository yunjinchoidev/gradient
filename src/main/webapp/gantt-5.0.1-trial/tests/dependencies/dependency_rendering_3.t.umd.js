...
StartTest(t => {
  ...
  t.it('Should render dependencies after zoom keeping center date', async t => {
    ...
    await t.waitForSelector('.b-sch-dependency'); // When we zoom to a time span (reconfigure time axis) dependencies feature will schedule a draw and will raise
  ...
  t.it('Should render dependencies after zoom NOT keeping center date', async t => {
    ...
    await Promise.all([t.waitForEvent(gantt, 'visibleRangeChange'), t.waitForEvent(gantt, 'dependenciesDrawn')]);
    t.selectorCountIs('.b-sch-dependency', 0, 'All dependencies are out of the view');
    await gantt.scrollTaskIntoView(gantt.taskStore.getById(1), {
      block: 'center'
    });
    await t.waitForSelector('.b-sch-dependency');
    t.assertDependency(gantt, gantt.dependencies[0]);
