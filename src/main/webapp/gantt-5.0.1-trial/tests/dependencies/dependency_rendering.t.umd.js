...
StartTest(t => {
  ...
  function assertHorizontalBreakOnRowBorder(t, dependencyId, rowId, expectedPoints = 6) {
    t.assertHorizontalBreakOnRowBorder(gantt, {
      dependencyId,
      rowId,
      expectedPoints
    });
  ...
  t.it('Dependency break line is aligned with row boundary (task to task)', async t => {
    gantt = t.getGantt({
    ...
    gantt.dependencies.forEach(dep => t.assertDependency(gantt, dep));
    assertHorizontalBreakOnRowBorder(t, 1, 12, 3);
    assertHorizontalBreakOnRowBorder(t, 2, 22, 3);
    assertHorizontalBreakOnRowBorder(t, 3, 31);
  });
  t.it('Dependency break line is aligned with row boundary (task/milestone)', async t => {
    gantt = await t.getGanttAsync({
      // Force no vertical overflow to get rid of random resize event which may or may not fire
      height: 1000,
      columns: [{
        type: 'name'
      }],
    ...
    gantt.dependencyStore.forEach(dep => t.assertDependency(gantt, dep));
  ...
  t.it('Should render dependencies correctly during dragdrop', t => {
    ...
    const [dep1, dep2] = gantt.dependencies;
    t.chain(gantt.depsAlreadyDrawn ? null : {
  ...
  t.it('Should render dependencies effectively on event change', async t => {
    // Dependencies are drawn twice on dependency model change. First draw from project refresh, second from endBatch
    // Don't see clear way around yet. Snooze test for a while.
    ...
    await event.setConstraintDate(new Date(2017, 0, 19));
    await gantt.await('dependenciesdrawn', {
      checkLog: false
    });
    await event.setConstraintDate(new Date(2017, 0, 20));
    await gantt.await('dependenciesdrawn', {
      checkLog: false
    });
    await event.setConstraintDate(new Date(2017, 0, 23));
    await gantt.await('dependenciesdrawn', {
      checkLog: false
    });
  });
  t.it('Should render dependencies effectively on dependency change', async t => {
    // Dependencies are drawn twice on dependency model change. First draw from project refresh, second from endBatch
    // Don't see clear way around yet. Snooze test for a while.
  ...
  t.it('Should render dependencies to tasks outside of the view', async t => {
    gantt = await t.getGanttAsync({
      tasks: [{
        id: 1,
        name: 'Task 1',
  ...
  t.it('Dependency line should hide, and not reappear when one end is indented', t => {
    ...
    const t1 = gantt.taskStore.getById(1),
    ...
    t.chain(gantt.depsAlreadyDrawn ? null : {
      ...
      t1.appendChild(t2);
      gantt.project.propagateAsync().then(next);
    }, next => {
