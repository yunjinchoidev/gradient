...
StartTest(t => {
  let gantt;
  t.beforeEach(() => {
    var _gantt, _gantt$destroy;

    return (_gantt = gantt) === null || _gantt === void 0 ? void 0 : (_gantt$destroy = _gantt.destroy) === null || _gantt$destroy === void 0 ? void 0 : _gantt$destroy.call(_gantt);
  });
  t.it('Should render dependencies regardless of barMargin size', async t => {
    ...
    t.chain({
      ...
      waitFor: () => document.querySelector(gantt.eventSelector).offsetHeight === 30,
      desc: 'Task height is correct after barMargin set to 10'
    }, next => {
      dependencies.forEach(dep => t.assertDependency(gantt, dep, undefined, `Dependency (${dep.id}) check after barMargin set to 10`));
      t.waitForEvent(gantt, 'dependenciesdrawn', next);
      gantt.barMargin = 20;
    }, {
      waitFor: () => document.querySelector(gantt.eventSelector).offsetHeight === 10,
      desc: 'Task height is correct after barMargin set to 20'
    }, () => {
  ...
  t.it('Should correctly draw dependencies on task add/remove', async t => {
    ...
    t.chain({
      ...
      await gantt.project.commitAsync(); // need to wait for stm to be in enabled state, because undo will turn it off/on
  ...
  t.it('Should avoid forced reflow during refresh', t => {
    ...
    function setupOverrides() {
      const feature = gantt.features.dependencies,
  ...
  t.it('Should clear dependencies cache when clearing task store', async t => {
    ...
    gantt.taskStore.add({});
    await gantt.project.commitAsync(); // Dependencies, if there are any, will schedule to next animation frame
  ...
  t.it('Should draw dependency line correctly after adding predecessor which pushes line far out of the view', async t => {
    ...
    t.chain({
      ...
      gantt.scrollTaskIntoView(gantt.taskStore.getById(11), {
        block: 'center'
      });
      await gantt.await('dependenciesDrawn', {
        checkLog: false
  ...
  t.it('Should clear dependency line after undo', async t => {
    ...
    await Promise.all([gantt.await('dependenciesDrawn', {
      checkLog: false
    }), gantt.project.await('dataReady', {
      checkLog: false
    })]);
    stm.stopTransaction();
    t.selectorCountIs('.b-sch-dependency[depId="1"]', 1, 'Dependency line for #1 is found');
