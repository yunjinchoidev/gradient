...
describe('Test buttons', function (t) {
  ...
  t.it('Check toolbar buttons', t => {
    ...
    const checkToolTip = (t, button, text, x, y, width, height) => [{
      moveMouseTo: button.element
    }, {
      waitForSelector: `.b-tooltip:contains(${text})`,
      desc: 'Correct tooltip shown'
    }];

    t.chain(checkToolTip(t, buttons.addTaskButton, 'Create new task', 0, 20, 127, 47), checkToolTip(t, buttons.editTaskButton, 'Edit selected task', 78, 20, 135, 47), {
  ...
  t.it('Gantt should fill the whole width of container', t => {
    const containerWidth = getComputedStyle(document.querySelector('app-gantt')).width,
