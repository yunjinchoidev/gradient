...
StartTest(async t => {
  ...
  t.it('Should add/update/remove new tasks/resource/assignments', async t => {
    try {
      ...
      await project.commitAsync(); // region Add

      t.diag('Add');
      const newResourceName = IdHelper.generateId('resource'),
        ...
        children: [{
          ...
          leaf: true
            ...
            [resource] = gantt.resourceStore.add({
        Name: newResourceName
      }),
            [assignment] = gantt.assignmentStore.add({
        TaskId: child.id,
      ...
      gantt.crudManager.on({
        ...
        beforeSyncApply({
          ...
          const parentId = response.tasks.rows[0].Id,
      ...
      gantt.crudManager.on({
        beforeSync({
          ...
          }, pack.tasks.updated.find(r => r.Id === parent.id), 'Parent update request is ok');
          t.isDeeplySubset({
            StartDate: new Date(2012, 8, 5, 8),
            EndDate: new Date(2012, 8, 11, 17),
            ConstraintDate: new Date(2012, 8, 5),
            ConstraintType: 'startnoearlierthan'
          }, pack.tasks.updated.find(r => r.Id === child.id), 'Child update request is ok');
        ...
        beforeSyncApply({
          ...
          t.isDeeply(response.tasks.rows.sort((a, b) => a.Id - b.Id), [{
            Id: parent.id
          }, {
            Id: child.id
      ...
      gantt.crudManager.on({
        beforeSync({
          ...
          t.isDeeply(pack.tasks.removed.sort((a, b) => a.Id - b.Id), [{
            Id: parent.id
          }, {
            Id: child.id
        ...
        beforeSyncApply({
          ...
          t.isDeeply(response.tasks.removed.sort((a, b) => a.Id - b.Id), [{
            Id: parent.id
          }, {
            Id: child.id
          }], 'Tasks are removed');
      ...
      await project.sync();
      t.notOk(gantt.resourceStore.find(r => r.name === updatedResourceName), 'Resource is not in the store'); // endregion
    } catch (e) {
      t.fail(e.stack);
    }
  });
  t.it('Should add/update/remove new tasks/resource/assignments', async t => {
    try {
      ...
      t.diag('Add');
      const parent1Name = IdHelper.generateId('record'),
      ...
      gantt.crudManager.on({
        beforeSync({
          ...
          }, pack.tasks.updated.find(r => r.Id === child11.id), 'Parent update request is ok');
          t.isDeeplySubset({
            parentId: parent1.id,
            $PhantomId: child21.id,
            $PhantomParentId: undefined
          }, pack.tasks.added.find(r => r.$PhantomId === child21.id), 'Task add request is ok');
        },
      ...
      await gantt.project.sync(); // endregion
    } catch (e) {
      t.fail(e.stack);
    }
  });
  t.it('Should add/update/remove new tasks/dependencies', async t => {
    try {
      ...
      t.diag('Add');
      const parent = gantt.taskStore.rootNode.appendChild({
        Name: 'New parent',
        expanded: true,
        children: [{
          Name: 'Child 1',
          ...
          leaf: true
            ...
            [dependency] = gantt.dependencyStore.add({
        From: child1.id,
        To: child2.id,
        Lag: 1
      });
      ...
      gantt.crudManager.on({
        ...
        beforeSyncApply({
          ...
          const child1Id = response.tasks.rows[1].Id,
      ...
      gantt.crudManager.on({
        beforeSync({
          ...
          t.isDeeplySubset({
            EndDate: new Date(2012, 8, 14, 17)
          }, pack.tasks.updated.find(r => r.Id === child2.id), 'Child update request is ok');
