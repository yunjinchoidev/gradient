StartTest(t => {
    ...
    t.it('Should edit child cost inline and recalculate parent cost', t =>   {
        t.chain(
            ...
            { waitForSelector : '.b-grid-cell:contains("$116750")', desc : 'Correct cost displayed' },
            {
                dblClick : '.b-grid-row[data-index="2"] .b-grid-cell[data-column="cost"]',
                desc     : 'Open cost column editor'
            },
            { waitForSelector : '.b-grid-cell[data-column="cost"].b-editing', desc : 'Editor displayed' },
            { type : '10[ENTER]', clearExisting : true, desc : 'Edit Cost' },
            async() => {
                t.is(task.cost, 116560, 'Correct cost calculated');
            },
            { waitForSelector : '.b-grid-cell:contains("$116560")', desc : 'Correct cost displayed' }
        );
    });
    ...
    t.it('Should edit child cost in Task edit dialog and recalculate parent cost', t =>   {
        t.chain(
            ...
            { type : '20', desc : 'Edit Cost', clearExisting : true },
            { click : '.b-button:contains(Save)' },
            async() => {
                t.is(task.cost, 116570, 'Correct cost calculated');
            },
            { waitForSelector : '.b-grid-cell:contains("$116570")', desc : 'Correct cost displayed' }
        );
    });
