...
StartTest(t => {
  ...
  let wasLoaded = false; // drop the database after the test finishes
  ...
  t.it('Task sort order should be stored on server', t => {
    ...
    {
      waitFor: () => task = gantt.taskStore.getById('12'),
      desc: 'Task #12 is found'
    }, {
      waitFor: () => task.parentIndex === 1,
      desc: 'Correct parent index after data reset'
    }, // scroll to top (seems gantt keeps scroll position after data loading)
    () => gantt.scrollable.scrollTo(null, 0), {
      waitForEvent: [gantt.project, 'haschanges'],
      trigger: {
  ...
  t.it('Should reset data', t => {
    t.chain({
      ...
      waitFor: () => gantt.taskStore.getById(4037),
  ...
  t.it('Should be able to drag newly created task', t => {
    ...
    t.chain({
      ...
      taskElement = gantt.getElementFromTaskRecord(gantt.taskStore.last);
      taskTop = taskElement.getBoundingClientRect().top;
      next();
    }, clickSaveButton, {
      waitForEvent: [gantt.project, 'haschanges'],
      trigger: {
      ...
      t.isNot(taskElement.getBoundingClientRect().top, taskTop, 'Task has moved up');
    });
  ...
  t.it('Reset data after TaskEditor is shown should work', t => {
    t.chain( // scroll to top (seems gantt keeps scroll position after data loading)
    () => gantt.scrollable.scrollTo(null, 0), // Reset data when Task Editor is open
    {
  ...
  t.it('Should not duplicate newly created and saved task', t => {
    t.chain({
      click: tools.addTaskButton.element
    }, clickSaveButton, () => t.selectorCountIs('.b-sch-label:contains(New task)', 1, 'Only one new task is rendered'));
  ...
  t.it('Should not throw error on task node removal', t => {
    ...
    () => gantt.scrollable.scrollTo(null, 0), {
      ...
      click: '.b-menuitem :textEquals(Delete)'
    }, clickSaveButton);
  });
