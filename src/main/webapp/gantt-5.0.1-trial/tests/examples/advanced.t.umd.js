...
describe('Test buttons', t => {
  const gantt = bryntum.query('gantt'),
        ...
        tools = tbar.widgetMap; // Make buttons a little smaller so that they do not overflow into the overflow menu so we can test them

  tbar.style = {
    fontSize: '80%'
  };
  !gantt.features.taskTooltip.isDestroyed && gantt.features.taskTooltip.destroy(); // https://github.com/bryntum/support/issues/1229
  ...
  t.it('Dependencies should not disappear when filter and update project start date and clear filter', async t => {
    t.chain({
      ...
      waitForSelector: 'polyline[depId=2]',
      desc: 'Dependency is visible after filter because source and target are visible'
    }, {
      ...
      waitForSelector: 'polyline[depId=2]',
      desc: 'Dependency is visible after filter and update start date'
    });
  });
  t.it('Check toolbar buttons', t => {
    ...
    () => gantt.editTask(gantt.taskStore.last), // rename task to bar
    {
  ...
  t.it('Should support turning features on and off', t => {
    t.chain( // make sure zoom level displays days in the bottom
    ...
    // schedule collapsing
    {
      ...
      click: '.b-menu-text:textEquals(Hide schedule)'
    }, async () => t.notOk(gantt.subGrids.normal.isCollapsed, 'Schedule expanded') // eof schedule collapsing
    );
  }); // Can't interact with native slider elements so calling listeners manually

  t.it('Should support changing UI settings', t => {
    tbar.onBarMarginChange({
      value: 10
    });
    t.is(gantt.barMargin, 10, 'Bar margin changed');
    tbar.onAnimationDurationChange({
      value: 1000
    });
    t.is(gantt.transitionDuration, 1000, 'Transition duration changed');
    tbar.onRowHeightChange({
      value: 40,
      source: tbar.widgetMap.featuresButton.menu.items[0].menu.widgetMap.rowHeight
    });
  ...
  t.it('Should translate columns and toolbar items', async t => {
    t.chain({
      ...
      waitForSelector: '.b-button[data-ref="addTaskButton"] label:contains(Создать)',
      desc: 'Button "Create" translated'
    }, {
      moveMouseTo: '.b-button[data-ref="addTaskButton"]'
    }, {
      waitForSelector: '.b-tooltip:contains(Создать новую задачу)',
      desc: 'Button "Create" tooltip translated'
    }, {
      click: '.b-button[data-ref="featuresButton"]'
    }, {
      waitForSelector: '.b-menuitem:contains(Зависимости задач)',
      desc: 'Menu item for "Features" button translated'
    }, {
      moveCursorTo: '.b-menuitem:nth-child(2)'
    }, {
      moveCursorTo: '.b-menuitem:nth-child(1)'
    }, {
      waitForSelector: '.b-slider[data-ref="rowHeight"]:contains(Высота строки)',
      desc: 'Slider field for "Row height" translated'
    }, {
      waitForSelector: '.b-grid-header[data-column="startDate"]:contains(Начало)',
      desc: 'Column "Start" translated'
    }, {
      waitForSelector: '.b-datefield[data-ref="startDateField"]:contains(Старт проекта)',
      desc: 'Label for "Project Start" translated'
    }, {
      waitForSelector: '.b-textfield[data-ref="filterByName"]:contains(Найти задачи по названию)',
      desc: 'Placeholder for "Filter" translated'
    }, // https://github.com/bryntum/support/issues/2118
    {
      moveMouseTo: '.b-button[data-ref="undoBtn"]'
    }, {
      waitForSelector: '.b-tooltip:contains(Отменить последнее действие)',
      desc: 'Button "Undo" tooltip translated'
    }, {
      click: '.b-button[data-ref="undoBtn"]'
    }, {
      moveMouseTo: '.b-button[data-ref="redoBtn"]'
    }, {
      waitForSelector: '.b-tooltip:contains(Повторить последнее отмененное действие)',
      desc: 'Button "Redo" tooltip translated'
    });
  });
});