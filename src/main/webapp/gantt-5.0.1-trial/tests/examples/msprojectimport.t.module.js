StartTest(t => {
    ...
    t.it('Check import of the mpp-file file shipped w/ the demo', t => {
        ...
        t.chain(
            ...
            { waitForSelector : '.b-mask-content:contains(Importing project)', desc : 'Import showed up' },
            { waitForSelectorNotFound : '.b-mask-content:contains(Importing project)', desc : 'Import mask disappeared' },
            { waitForSelector : '.b-toast:contains(File imported successfully)', desc : 'Final toast showed up' },

            { diag : 'Asserting columns' },
            { waitForSelector : '.b-grid-header:contains(Name)', desc : '`Name` column is imported' },
            { waitForSelector : '.b-grid-header:contains(Duration)', desc : '`Duration` column is imported' },
            { waitForSelector : '.b-grid-header:contains(Start)', desc : '`Start` column is imported' },
            { waitForSelector : '.b-grid-header:contains(Finish)', desc : '`Finish` column is imported' },
            { waitForSelector : '.b-grid-header:contains(Assigned Resources)', desc : '`Assigned resources` column is imported' },
            { waitForSelector : '.b-grid-header:contains(% Done)', desc : '`% done` column is imported' },
