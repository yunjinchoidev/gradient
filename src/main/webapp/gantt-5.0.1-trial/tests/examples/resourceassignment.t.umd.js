...
StartTest(t => {
  ...
  t.it('Should handle selected rows in collapsed groups', async t => {
    t.chain( // First block just selects one resource, then collapses the group it's in before saving
    {
      ...
      offset: [30, 36]
    }, async () => t.selectorExists('.b-resource-avatar[src*=celia]'), // Now we verify picker
    {
      dblclick: '[data-id="15"] .b-resourceassignment-cell'
      ...
      click: '.b-grid-cell:contains(Barcelona)'
    }, async () => t.selectorExists('.b-grid-row:not(.b-group-row) .b-check-cell input:checked', 'Celia checked'), async () => t.selectorExists('.b-grid-row:not(.b-group-row) [data-column=resourceName]:contains(Celia)', 'Celia text present'), {
