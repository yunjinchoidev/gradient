import { AssignmentGrid, ProjectModel } from '../../build/gantt.module.js?457330';
...
StartTest((t) => {
    ...
    t.it('Should render properly', async(t) => {
        ...
        const
            arcadyAssignment = grid.store.find(a => a.resourceName === 'Arcady'),
    ...
    t.it('Should be possible to bulk assign/unassign resources', async(t) => {
        ...
        t.is(grid.selectedRecords.length, 0, 'All resources are unassigned');

        // Now let's assign all by bulk assignment/unassignment checkbox
        await new Promise((resolve) => {
            t.chain(
                { click : assignAllCb },
                resolve
    ...
    t.it('Should be possible to edit assignment units', async(t) => {
        ...
        // - Nick(8) 10%
        const
            arcadyAssignment = grid.store.find(as => as.resourceName === 'Arcady'),
            [arcadyUnitCell] = t.query('div[data-index=0] div[data-column=units]');
    ...
    t.it('Should show < > characters in name properly', async t => {
        ...
        project.resourceStore.data = [{ id : 1, name : '<name>' }];
