import { Rectangle, Container, Timeline } from '../../build/gantt.module.js?457330';
...
StartTest(t => {
    ...
    t.it('Should show timeline start / end even if no tasks are marked showInTimeline', async t => {
        ...
        t.chain(
            { waitForContentLike : ['.b-timeline-startdate', timeline.getFormattedDate(timeline.startDate)], desc : 'Start date label has correct value' },
            { waitForContentLike : ['.b-timeline-enddate', timeline.getFormattedDate(timeline.endDate)], desc : 'End date label has correct value' }
        );
    });
    ...
    t.it('Should position milestone correctly', async t => {
        ...
        const
            [task1, task2] = timeline.project.eventStore.getRange(),
    ...
    t.it('Should render to container', async t => {
        const project = t.getProject({
            eventsData : [
                { id : 1, startDate : '2021-04-28', duration : 2, showInTimeline : true }
            ]
        });
